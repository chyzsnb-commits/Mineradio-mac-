...
const {
  ...
  GOOGLE_GTX_URL,
} = require('./server/translation-service');
const PRIMARY_LYRIC_TRANSLATE_URL = GOOGLE_GTX_URL; // https://translate.googleapis.com/translate_a/single
...
const MINERADIO_USER_DATA_DIR = process.env.MINERADIO_USER_DATA_DIR || (
  process.platform === 'darwin'
    ? path.join(os.homedir(), 'Library', 'Application Support', 'Mineradio')
    : process.platform === 'win32'
      ? path.join(process.env.APPDATA || os.homedir(), 'Mineradio')
      : path.join(process.env.XDG_CONFIG_HOME || path.join(os.homedir(), '.config'), 'Mineradio')
);
const KUGOU_COOKIE_FILE = path.join(MINERADIO_USER_DATA_DIR, '.kugou-cookie');
...
const BEATMAP_CACHE_DIR = process.env.MINERADIO_BEAT_CACHE_DIR || (
  process.platform === 'win32'
    ...
    : path.join(MINERADIO_USER_DATA_DIR, 'beatmaps')
);
const APP_PACKAGE = readPackageInfo();
const APP_VERSION = process.env.MINERADIO_VERSION || APP_PACKAGE.version || '0.9.11';
const PLATFORM_INSTALLER_EXT = process.platform === 'darwin' ? 'dmg' : 'exe';
const PLATFORM_INSTALLER_SUFFIX = process.platform === 'darwin' ? `-${process.arch}.dmg` : '-Setup.exe';
const PLATFORM_INSTALLER_PATTERN = process.platform === 'darwin' ? /\.dmg$/i : /\.(exe|msi)$/i;
const PLATFORM_UPDATE_METADATA = process.platform === 'darwin' ? 'latest-mac.yml' : 'latest.yml';
...
// ====================================================================
//  酷狗音乐 — Provider Adapter
//  - 搜索 (search)
//  - 歌曲播放链接 (song url / audio stream)
//  - 用户歌单 (user playlists / playlist tracks)
// ====================================================================

function requireOptionalProvider(provider, name) {
...
const KUGOU_SONG_URL_BASE = 'https://wwwapi.kugou.com/yy/index.php';
const KUGOU_TRACK_LINK_URL = 'https://trackercdn.kugou.com/i/v2/song/track_link';
const KUGOU_USER_PLAYLIST_URL = 'https://wwwapi.kugou.com/yy/index.php';
const KUGOU_HEADERS = {
  'User-Agent': UA,
  Referer: 'https://www.kugou.com/',
};
const KUGOU_H5_SIGNATURE_SECRET = 'NVPh5oo715z5DIWAeQlhMDsWXXQV4hwt';
 ...
 */
function kugouGenerateSignature(params, secret) {
  const normalized = Object.keys(params || {})
    .filter(key => params[key] !== undefined && params[key] !== null && key !== 'signature' && key !== 'sign')
    .sort()
    .map(key => `${key}=${params[key]}`)
    .join('&');
  const key = secret || KUGOU_H5_SIGNATURE_SECRET;
  return crypto.createHash('md5').update(`${key}${normalized}${key}`).digest('hex');
}

function kugouClientParams(cookieObj) {
  cookieObj = cookieObj || {};
  const now = Date.now();
  const mid = cookieObj.kg_mid || cookieObj.mid || String(now);
  return {
    srcappid: '2919',
    clientver: '20000',
    clienttime: now,
    mid,
    uuid: cookieObj.uuid || mid,
    dfid: cookieObj.kg_dfid || cookieObj.dfid || '-',
  };
}

function logProviderHttpFailure(provider, operation, error) {
  const status = Number(error && error.statusCode) || 0;
  const body = String(error && error.body || '').slice(0, 2000);
  console.error(`[${provider}:${operation}] upstream failed`, {
    status,
    message: error && error.message || 'Unknown upstream error',
    body,
  });
}

function parseProviderJson(provider, operation, text) {
  try {
    return JSON.parse(text);
  } catch (cause) {
    const error = new Error(`Invalid JSON from ${provider} ${operation}`);
    error.body = String(text || '').slice(0, 2000);
    error.cause = cause;
    logProviderHttpFailure(provider, operation, error);
    throw error;
  }
}

async function kugouFetchJson(targetUrl, operation, options) {
  const headers = { ...KUGOU_HEADERS, ...((options && options.headers) || {}) };
  if (kugouCookie && (!options || options.cookie !== false)) headers.Cookie = kugouCookie;
  try {
    const text = await requestText(targetUrl, { headers });
    return parseProviderJson('Kugou', operation, text);
  } catch (error) {
    logProviderHttpFailure('Kugou', operation, error);
    throw error;
  }
}

async function kugouFetchText(targetUrl, operation, options) {
  const headers = { ...KUGOU_HEADERS, ...((options && options.headers) || {}) };
  if (kugouCookie && (!options || options.cookie !== false)) headers.Cookie = kugouCookie;
  try {
    return await requestText(targetUrl, { headers });
  } catch (error) {
    logProviderHttpFailure('Kugou', operation, error);
    throw error;
  }
...
function mapKugouSong(item) {
  ...
  // 酷狗封面: 使用 AlbumID 构造标准封面URL
  let cover = '';
  if (albumId) {
    cover = `https://img1.kgimg.com/stdmusic/${Math.floor(albumId / 1000000)}/${albumId}/cover.jpg`;
  }
  if (!cover && item.album_img) cover = item.album_img;
  if (!cover && item.imgUrl) cover = item.imgUrl;

  // 歌手信息
  const artistRaw = item.SingerName || item.singer_name || item.author || '';
  const artists = artistRaw ? artistRaw.split(/[,、\/]/).map((n, i) => ({
    id: (item.SingerList && item.SingerList[i] && item.SingerList[i].SingerMID) || '',
    name: n.trim(),
  })).filter(a => a.name) : [];
  ...
  return {
    ...
    source: 'kugou',
    type: 'song',
    id: String(item.Hash || item.hash || item.FileHash || ''),
    name: item.SongName || item.song_name || item.FileName || '',
    artist: artists.map(a => a.name).join(' / ') || artistRaw,
    artists,
    artistId: artists[0] && artists[0].id,
    album: albumName,
    cover,
    duration: Number(item.Duration || item.duration || 0) * 1000 || 0,
    fee: 0, // 酷狗大部分歌曲免费可播
    // 酷狗特有字段
    hash: item.Hash || item.hash || item.FileHash || '',
    albumId,
 ...
 * @returns {Promise<Array>} 统一格式的歌曲列表
 */
async function kugouSearch(keywords, limit) {
  ...
  // 酷狗公开 Web 搜索端点。复杂搜索端点要求随版本变化的私有签名，
  // 不再伪造签名；该端点由酷狗 Web 搜索页直接使用且返回稳定 JSON。
  const params = {
    keyword: kw,
    page,
    pagesize: pageSize,
  ...
  const data = await kugouFetchJson(u.toString(), 'search');
  const list = data && data.data && data.data.lists;
  if (!Array.isArray(list)) return [];

  return list.map(mapKugouSong).filter(s => s.id && s.name);
}

/**
 * 酷狗 - 通过 Hash 获取真实音频播放链接
 * 支持多种音质探测，自动降级
 * @param {string} hash - 歌曲 MD5 Hash
 * @returns {Promise<Object>} {url, playable, quality, ...}
 */
async function kugouGetSongUrl(hash) {
  const songHash = String(hash || '').trim();
  if (!songHash) return { provider: 'kugou', url: '', playable: false, error: 'MISSING_HASH', message: '缺少酷狗歌曲 Hash' };

  console.log('[KugouSongUrl] hash:', songHash);

  // 音质优先级: 高品 → 标准 → 低码 → 手机
  const qualities = [
    { key: 'hash_320', label: '高品', ext: '.mp3' },
    { key: 'hash', label: '标准', ext: '.mp3' },
    { key: 'hash_128', label: '低码', ext: '.mp3' },
  ];

  // 尝试使用 trackercdn 接口（更稳定）
  try {
    const trackUrl = `${KUGOU_TRACK_LINK_URL}?cmd=25&pid=1&force_play=1&hash=${encodeURIComponent(songHash)}`;
    const trackData = await kugouFetchJson(trackUrl, 'track-link', {
      headers: {
        'KG-RCF': 'v1',
      },
    });
    if (trackData && trackData.url && trackData.status === 1) {
      return {
        provider: 'kugou',
        url: trackData.url,
        playable: true,
        trial: false,
        quality: trackData.bitRate ? `${Math.round(trackData.bitRate / 1000)}kbps` : '标准',
        level: 'standard',
        hash: songHash,
        fileName: trackData.fileName || '',
        audioName: trackData.audioName || '',
        size: trackData.size || 0,
        bitRate: trackData.bitRate || 0,
        timeLength: trackData.timeLength || 0,
      };
    }
  } catch (e) {
    console.warn('[KugouSongUrl] trackercdn failed:', e.message);
  }

  // 降级方案：使用 yy/index.php 接口
  for (const q of qualities) {
    try {
      const u = new URL(KUGOU_SONG_URL_BASE);
      u.searchParams.set('r', 'play/getdata');
      u.searchParams.set('hash', songHash);
      u.searchParams.set('mid', '1');
      u.searchParams.set('platid', '4');

      const data = await kugouFetchJson(u.toString(), 'song-url');
      if (data && data.data && data.data.play_url) {
        // 有些返回的是 base64 编码的 URL
        let playUrl = data.data.play_url;
        if (/^[A-Za-z0-9+/]+=*$/.test(playUrl) && !playUrl.startsWith('http')) {
          try { playUrl = Buffer.from(playUrl, 'base64').toString('utf8'); } catch (_) {}
        }
        if (playUrl && playUrl.startsWith('http')) {
          return {
            ...
            level: q.key,
            hash: songHash,
            audioName: data.data.audio_name || '',
            size: data.data.size || 0,
            bitrate: data.data.bitRate || 0,
            duration: data.data.time_length || 0,
          };
        }
      }
    } catch (e) {
      console.warn(`[KugouSongUrl] ${q.key} failed:`, e.message);
    }
  }
  ...
  return {
    ...
    message: '酷狗无法获取该歌曲的播放链接，可能版权受限或需要VIP',
    hash: songHash,
  };
}
...
/**
 * 酷狗 - 获取用户创建的歌单列表
 * @returns {Promise<Object>} {loggedIn, playlists}
 */
async function kugouUserPlaylists() {
  if (!kugouCookie) return { loggedIn: false, provider: 'kugou', playlists: [] };

  try {
    const cookieObj = parseCookieString(kugouCookie);
    const userId = cookieObj.KugooID || cookieObj.kugouid || '';
    if (!userId) return { loggedIn: false, provider: 'kugou', playlists: [], error: 'MISSING_USER_ID' };

    console.log('[KugouPlaylists] userId:', userId);

    // 获取用户创建的歌单
    const u = new URL(KUGOU_USER_PLAYLIST_URL);
    u.searchParams.set('r', 'user/getgroup');
    u.searchParams.set('userid', userId);
    u.searchParams.set('platid', '4');
    const clientParams = kugouClientParams(cookieObj);
    Object.keys(clientParams).forEach(key => u.searchParams.set(key, String(clientParams[key])));
    u.searchParams.set('signature', kugouGenerateSignature(Object.fromEntries(u.searchParams.entries())));

    const text = await kugouFetchText(u.toString(), 'user-playlists');
    const data = parseProviderJson('Kugou', 'user-playlists', text);
    if (Number(data && (data.err_code || data.error_code || data.status)) < 0 || data && data.error_code) {
      const error = new Error(data.error_msg || data.message || 'Kugou playlist request rejected');
      error.statusCode = Number(data.error_code || data.err_code || 0);
      error.body = text;
      logProviderHttpFailure('Kugou', 'user-playlists', error);
      throw error;
    }
    const list = data && data.data && data.data.info && data.data.info.group;

    if (!Array.isArray(list)) return { loggedIn: true, provider: 'kugou', userId, playlists: [] };

    const playlists = [];
    list.forEach(group => {
      const gInfo = group || {};
      const gName = gInfo.group_name || gInfo.groupname || '';
      const gCreateType = Number(gInfo.create_type || 0); // 1=收藏 0=创建
      const items = Array.isArray(gInfo.disslist) ? gInfo.disslist : [];

      items.forEach(pl => {
        playlists.push({
          provider: 'kugou',
          source: 'kugou',
          id: String(pl.dissid || pl.specialid || pl.id || ''),
          name: pl.dissname || pl.name || pl.title || '',
          cover: pl.img || pl.cover || pl.pic || '',
          trackCount: Number(pl.songcount || pl.song_count || pl.songnum || 0),
          creator: pl.creatorname || pl.nickname || gName || '酷狗用户',
          subscribed: gCreateType === 1,
          specialType: 0,
          groupName: gName,
          createTime: pl.createtime || pl.create_time || 0,
          updateTime: pl.updatetime || pl.update_time || 0,
        });
      });
    });

    return { loggedIn: true, provider: 'kugou', userId, playlists: playlists.filter(p => p.id && p.name) };
  } catch (err) {
    console.error('[KugouPlaylists]', err);
    return { loggedIn: !!kugouCookie, provider: 'kugou', error: err.message, playlists: [] };
  }
}
 ...
 */
async function kugouPlaylistTracks(playlistId) {
  const pid = String(playlistId || '').trim();
  if (!pid) return { provider: 'kugou', error: 'Missing playlist id', tracks: [], playlist: {} };

  if (!kugouCookie) return { loggedIn: false, provider: 'kugou', tracks: [], playlist: { id: pid }, error: 'LOGIN_REQUIRED' };

  try {
    console.log('[KugouPlaylistTracks] id:', pid);

    // 获取歌单详情和曲目列表
    const u = new URL(KUGOU_USER_PLAYLIST_URL);
    u.searchParams.set('r', 'playlist/get');
    u.searchParams.set('listid', pid);
    u.searchParams.set('platid', '4');
    const clientParams = kugouClientParams(parseCookieString(kugouCookie));
    Object.keys(clientParams).forEach(key => u.searchParams.set(key, String(clientParams[key])));
    u.searchParams.set('signature', kugouGenerateSignature(Object.fromEntries(u.searchParams.entries())));

    const data = await kugouFetchJson(u.toString(), 'playlist-tracks');
    const info = data && data.data && data.data.info;
    const rawTracks = data && data.data && data.data.tracks;

    const playlistMeta = info ? {
      provider: 'kugou',
      id: pid,
      name: info.specialname || info.name || info.dissname || '',
      cover: info.img || info.cover || info.pic || '',
      trackCount: Number(info.songcount || info.song_count || 0),
      creator: info.username || info.nickname || info.creatorname || '',
      createTime: info.createtime || 0,
      updateTime: info.updatetime || 0,
      intro: info.intro || info.desc || '',
    } : { id: pid, name: '', trackCount: 0 };

    const tracks = (Array.isArray(rawTracks) ? rawTracks : []).map(mapKugouSong).filter(s => s.id && s.name);

    if (!playlistMeta.trackCount) playlistMeta.trackCount = tracks.length;

    return { loggedIn: true, provider: 'kugou', playlist: playlistMeta, tracks };
  } catch (err) {
    console.error('[KugouPlaylistTracks]', err);
    return { provider: 'kugou', error: err.message, tracks: [], playlist: { id: pid } };
  }
}
...
const KUGOU_API = {
  search: kugouSearch,
  getSongUrl: kugouGetSongUrl,
  getUserPlaylists: kugouUserPlaylists,
  getPlaylistTracks: kugouPlaylistTracks,
};

function savePrivateCredential(filePath, value) {
...
function saveProviderCookie(provider, value) {
  if (provider !== 'kugou') throw new Error(`Unsupported provider cookie: ${provider}`);
  const normalized = normalizeCookieHeader(value) || rawCookieFallback(value);
  kugouCookie = normalized;
  savePrivateCredential(KUGOU_COOKIE_FILE, normalized);
  return normalized;
}
...
function pickReleaseAsset(assets) {
  ...
  const preferred = list.find(a => PLATFORM_INSTALLER_PATTERN.test(a && a.name || ''))
...
function normalizeManifestUpdateInfo(data) {
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `Mineradio-${latestVersion}${PLATFORM_INSTALLER_SUFFIX}`,
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `Mineradio-${latestVersion}${PLATFORM_INSTALLER_SUFFIX}`;
...
async function fetchLatestYmlUpdateInfo(reason) {
  ...
  const latestYmlUrl = `https://github.com/${encodeURIComponent(UPDATE_CONFIG.owner)}/${encodeURIComponent(UPDATE_CONFIG.repo)}/releases/latest/download/${PLATFORM_UPDATE_METADATA}`;
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `Mineradio-${version || APP_VERSION}.${PLATFORM_INSTALLER_EXT}`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `Mineradio-${version || APP_VERSION}.${PLATFORM_INSTALLER_EXT}`;
}
...
function startUpdatePatchJob(info) {
  ...
  const job = {
    ...
    total: patch.size || 0,
    mode: 'patch',
    fileName: patch.name || safeUpdateFileName('', version).replace(/\.(exe|dmg)$/i, '.patch.json'),
    filePath: '',
    version,
    downloadUrl,
    downloadCandidates,
...
function normalizeKugouLoginInfo(cookieText) {
  const obj = parseCookieString(cookieText);
  const userId = obj.KugooID || '';
  const token = obj.token || obj.KugooToken || obj.KuGoo || '';
  return {
    provider: 'kugou',
    loggedIn: !!(userId && token),
    userId,
    nickname: obj.NickName || obj.nickname || (userId ? `酷狗用户 ${userId}` : '酷狗音乐'),
    avatar: obj.Pic || obj.avatar || '',
    vipType: Number(obj.vip_type || obj.vipType || 0) || 0,
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/kugou/search') {
    try {
      ...
      const songs = await requireOptionalProvider(KUGOU_API, 'Kugou').search(kw, limit);
      sendJSON(res, { provider: 'kugou', songs });
    } catch (err) {
      console.error('[KugouSearch]', err);
      sendJSON(res, { provider: 'kugou', error: err.message, songs: [] }, err.code === 'PROVIDER_NOT_CONFIGURED' ? 501 : 500);
    }
    return;
  }
  ...
  if (pn === '/api/kugou/song/url') {
    try {
      ...
      const info = await requireOptionalProvider(KUGOU_API, 'Kugou').getSongUrl(hash);
      sendJSON(res, { provider: 'kugou', ...info });
    } catch (err) {
      console.error('[KugouSongUrl]', err);
      sendJSON(res, { provider: 'kugou', url: '', playable: false, error: err.message }, err.code === 'PROVIDER_NOT_CONFIGURED' ? 501 : 500);
    }
    return;
  }

  const providerLoginRoutes = {
    kugou: {
      cookie: '/api/kugou/login/cookie',
      status: '/api/kugou/login/status',
      logout: '/api/kugou/logout',
    },
  };
  for (const provider of ['kugou']) {
    const routes = providerLoginRoutes[provider];
    const cookieText = kugouCookie;
    const normalize = normalizeKugouLoginInfo;
    if (pn === routes.status) {
      sendJSON(res, normalize(cookieText));
      return;
    }
    if (pn === routes.cookie && req.method === 'POST') {
      try {
        const body = await readRequestBody(req);
        const normalized = normalizeCookieHeader(body.cookie || body.data || body.text || '');
        const info = normalize(normalized);
        if (!info.loggedIn) {
          sendJSON(res, { ...info, error: `INVALID_${provider.toUpperCase()}_COOKIE`, message: '酷狗登录凭证不完整' }, 400);
          return;
        }
        saveProviderCookie(provider, normalized);
        sendJSON(res, { ...info, saved: true });
      } catch (e) {
        sendJSON(res, { provider, loggedIn: false, error: e.message }, 500);
      }
      return;
    }
    if (pn === routes.logout) {
      saveProviderCookie(provider, '');
      sendJSON(res, { provider, ok: true, loggedIn: false });
      return;
    }
  }
  ...
  if (pn === '/api/kugou/user/playlists') {
    try {
      const data = await kugouUserPlaylists();
      sendJSON(res, data);
    } catch (err) {
  ...
  if (pn === '/api/kugou/playlist/tracks') {
    try {
      const id = url.searchParams.get('id') || url.searchParams.get('dissid') || '';
      const data = await kugouPlaylistTracks(id);
      sendJSON(res, data);
    } catch (err) {
      console.error('[KugouPlaylistTracks]', err);
  ...
  if (pn === '/api/translate/lyrics') {
    try {
      const body = req.method === 'POST' ? await readRequestBody(req) : Object.fromEntries(url.searchParams.entries());
      const text = String(body.text || body.q || '').trim();
      const to = String(body.to || 'zh-CN').trim() || 'zh-CN';
      const from = String(body.from || 'auto').trim() || 'auto';
      const linePayload = Array.isArray(body.lines) ? body.lines : null;
      const sourceLines = linePayload && linePayload.length
        ? linePayload.map((line, index) => ({
            text: String(line && line.text || '').trim(),
            subText: String(line && line.subText || '').trim(),
            index: Number(line && line.index),
          }))
        : text.split(/\r?\n/).map((line, index) => ({ text: String(line || '').trim(), subText: '', index }));
      if (!text && !(sourceLines && sourceLines.some(line => line.text))) {
        sendJSON(res, { error: 'Missing text', translatedText: '', translatedLines: [] }, 400);
        return;
      }
      const result = await translateLyricLineSet(sourceLines, {
        from,
        to,
        userAgent: UA,
        fetchWithTimeout,
        logger: console,
        engines: createDefaultTranslationEngines({ userAgent: UA, fetchWithTimeout, logger: console }),
      });
      sendJSON(res, {
      ...
      console.error('[LyricTranslate]', {
        message: err && err.message || 'Unknown error',
        status: err && err.statusCode || 0,
        body: String(err && err.body || '').slice(0, 1000),
      });
      sendJSON(res, { error: err.message || 'Translate failed', translatedText: '', translatedLines: [], partial: true, failedBatches: [] }, 500);
    }
    return;
  }
