...
- 水滴保留五态物理模型（pinned/growing/breaking/slipping/settling）、面积守恒融合、蜿蜒滑落、残留微珠、接触角扰动、折射、菲涅尔、接触阴影和局部高光；同步独立页最新的尺寸与非线性流速校准。
- 动态面板新增 `玻璃水珠` 开关，以及 `水珠数量`（0.25–1.5）、`水珠流速`（0.2–2.2）、`水珠尺寸`（0.6–1.8）；独立存入 `mineradio-rain-toggles-v1`。尺寸只改变半径，数量不变。
- 关闭、切出预设、WebGL 上下文恢复或后处理失败时释放 GPU 资源；失败回退原始雨境雨丝渲染。
- 验证：新增 4 项后处理测试，雨境相关共 9 项通过；`npm run check` **174/174**；Electron 本地启动成功，页面 Console 0 errors（保留 1 条既有 warning）。
...
- [x] **雨境玻璃水珠迁移**：独立 RG Metaball 后处理已接入预设 9；新增开关、数量、流速、尺寸控件和独立持久化；背景保持锐利；`npm run check` 174/174。
