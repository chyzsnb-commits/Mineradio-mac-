...
- **2026-08-10 P10 一级歌架同步普通预设布局（未构建 DMG）：** 用户明确要求 P10 歌架不要继续使用独立的“近似”尺寸或角度，而是与其他预设完全一致。根因是 `p10ShelfSideLayout()` 额外重写了位置、纵深、进场位移、间距和 `0.82` 缩放，`p10ShelfRootPose()` 又叠加固定俯仰/偏航/滚转，导致同一歌架在 P10 中始终偏小且偏斜。现 P10 直接复用 `shelfLayoutProfile()` 的原布局对象；根组仅使用普通预设同源的指针姿态公式。P10 仍保留相机前方安全锚点、稳定世界比例、右键一级状态机、原生镜头半径/高度、歌词让位和冷色玻璃卡面，避免此前的钻入音柱、纯黑和二级跳转回归。先写“布局对象完全相同”的失败测试，再实施；专项 `21/21`、完整 `npm run check` `271/271`、`git diff --check` 通过，命令行 Electron 已从本工作树启动且主页 `200`。**下一步：** 用户在 P10 右键，确认一级歌架与其他预设的竖向尺寸/角度一致；点击一级卡确认才进入二级，并检查歌词不被遮挡。[来源: `public/js/modules/04-shelf/00-layout-hover.js`、`public/js/modules/04-shelf/01-manager-core.js`、`scripts/test-p10-voxel-interactions.js`]
