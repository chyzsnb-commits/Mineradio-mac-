...
> **词境穿行参考视频预设（2026-08-13）：** 新增 P11“词境穿行”，参考 `/Users/allenli/Desktop/抖音2026813-294011.mp4` 精确拆帧实现深空封面与五层景深歌词。歌词不是逐字打字：新句整组在 0.82s 左右从深处以小尺寸、低透明、失焦状态推到焦面；旧句连续越过焦面放大、失焦并降到低透明，与新句短暂重叠。视频里的白色手掌确认是网页 pointer 光标，没有可分离的封面/歌词 hover 放大、倾斜、发光或排斥，因此只复刻手形光标，不臆造互动。P11 的歌单完全复用音域回响的普通 DOM 歌单页；3D 歌单架的渲染、命中、点击、右键和滚轮均禁用，P10/P11 共享 dock 状态确保互切不闪回，同时保持用户原有的 3D 歌单架固定状态，歌单页只在对应 tab 激活时显示。歌词开关、无封面清理和进度拖动节流均补齐；拖动时约每 90ms 至多上传一张当前歌词纹理，松手再补齐五层。隔离 Electron 实测歌单双向切换及退出归位、tab 互斥显示、固定状态保留、歌词关闭、无封面、快速拖动和转场连续性通过；完整检查当前 **187/187**。功能提交 `d868954`，PR #115；arm64/x64 CI 构建成功，`codex-review` 因仓库 action 缺失 server-info 文件而未实际运行。唯一 `/Applications/Mineradio.app` 已无损迁移安装，`app.asar` SHA-256 为 `aab448ea1895b03609c76dab09675b4e0711f2a294bd920050d0f0325aae22c0`；真实 QQ 歌曲实测 44 行歌词、4 层可见景深卡、封面正常，预设保持为 P11。QQ 登录和播放密钥连续两次重开均有效，无 SecurityAgent/钥匙串弹窗；安装前 App 与完整 userData 备份在 `/Users/allenli/Desktop/Mineradio-2.0-backups.noindex/before-performance-cover-20260813-071515-lyric-depth`。Obsidian 指定路径在本机不存在，待仓库主人侧同步。
