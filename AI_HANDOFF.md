...
- **当前 Codex 任务链**：PR #27（显卡模式与快速启动）→ PR #29（Mac 真实显卡占用）→ PR #30（主循环真正休眠）→ PR #31（唱歌模式省电）均为叠加关系；当前分支 `codex/mac-memory-safe-auto` 基于 #31，设计存档 `01e4dea`、功能存档 `669348e`，实现 Mac 播放中分级自动清理，待创建叠加 PR。
...
- 分支：`codex/mac-memory-safe-auto`；基于 PR #31；设计 commit（代码存档点）`01e4dea`，功能 commit `669348e`，待创建叠加 PR。
