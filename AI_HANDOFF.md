...
- [ ] **快速切歌异步竞态**：连续点下一首时 UI 与实际音频必须一致，旧音源不能继续播放。
- [ ] **连续拖进度条异步竞态**：AI / 实时模式快速拖动后必须恢复声音并继续播放。
