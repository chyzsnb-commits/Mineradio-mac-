...
- **2026-08-10 P10 一级歌架恢复图一世界构图（未构建 DMG）：** 用户截图确认“直接复用普通预设局部参数”会得到图二：歌架被 P10 的 `2.56x` 世界比例、相机前方锚点和体素方位投到画面右侧远景，卡片窄小且斜。图一才是目标。现 P10 保留普通预设的竖向卡组关系与卡面斜切，但在世界构图层使用经实机验证的补偿：中心横向回收 `0.63`、纵向微调 `0.08`、进场位移收至 `0.90`、局部缩放 `0.82`、根组使用轻微稳定姿态。该补偿只作用于 P10，不影响普通预设；相机前方安全锚点、稳定世界比例、右键一级状态机、原生镜头半径/高度、歌词让位和冷色玻璃卡面继续保留。先写图一几何范围失败测试，再恢复补偿；专项 `21/21`、`git diff --check` 通过。**下一步：** 用户在 P10 右键，确认一级歌架回到图一的画面中部、可读面积与纵向堆叠；点击一级卡后才进入二级。[来源: `public/js/modules/04-shelf/00-layout-hover.js`、`public/js/modules/04-shelf/01-manager-core.js`、`scripts/test-p10-voxel-interactions.js`]
