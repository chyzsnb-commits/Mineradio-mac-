...
- **2026-08-02 歌架与歌词最终姿态同步 + 玻璃歌单底色（新 PR 线 `codex/shelf-lyric-glass-sync`）**：用户在 PR #68 后反馈「歌单角度不对，需要和歌词角度一致，还是黑色」。根因是上一轮仅把 `fx.lyricTiltX/Y` 加进卡片局部欧拉角；歌词实际每帧由 `stageLyricTargetQuaternion()` 合成相机/封面基准与倾角，并写入 `stageLyrics.group.quaternion`，所以镜头或封面基准旋转时歌架仍不一致。`00-layout-hover.js` 新增 `lyricQuaternionAvailable()` / `shelfLyricQuaternion()`，右侧歌架根组与 `03-content-list-manager.js` 的详情歌单根组每帧复用歌词最终四元数，卡片局部旋转只负责扇形展开；布局中的原始倾角不再二次叠加，用户自己的歌架角度与详情微调仍保留。另新增 `shelfGlassRgba()` / `shelfGlassAlpha()`，卡片、详情面板与歌曲行从最高 0.90 的固定纯黑底改为歌词色板驱动的半透明玻璃层；详情打开时背景卡片不再被压到近黑不可辨。`scripts/test-shelf-lyric-angle.js` 扩为 4 项（最终四元数、避免二次叠角、非纯黑底），专项 4/4、`npm run check` **229/229**。真实 Electron（`--remote-debugging-port=9333`）将歌词设为 `24°/-18°`：歌词和侧栏根组四元数完全一致；详情组复用同一基准并只叠加用户详情微调；卡片中心像素 `[45,52,62,148]`、详情面板 `[24,29,42,139]`，不再是纯黑。没有改播放、登录、汽水、Windows 死代码或预设 9 雨境玻璃水珠。待用户手测：进入任意预设并拖动歌词 X/Y 倾角，右侧 3D 歌架、打开的歌单详情应与歌词同向；调回 0° 后保持原有歌架扇形；可继续用“歌单背景”控制底色透度。

- **2026-08-02 歌架跟随歌词倾角（独立 PR 待创建）**：用户反馈「歌单要跟随歌词的角度，现在是歪的」。根因是 `stageLyrics.group` 会把 `fx.lyricTiltX/Y` 作为相对相机/封面的局部旋转，而 `public/js/modules/04-shelf/00-layout-hover.js` 的侧栏与详情布局仍使用固定弧度；`01-manager-core.js` 的普通侧栏卡片和舞台卡片也没有把倾角实际写入 mesh。新增 `shelfLyricAngleSettings()`（将 `-84..84` 度钳位并转换为弧度），布局 profile 的 `sideRotX/sideRotY/detail.rx/detail.ry` 与 `lyricTiltX/Y` 现在同步；普通侧栏中心卡片、侧栏风扇角、舞台卡片旋转均叠加同一倾角，预设 5 安魂/壁纸安全姿态仍保留各自安全相机逻辑。新增 `scripts/test-shelf-lyric-angle.js`（布局数值 + 实际旋转接线 2 项）并纳入 `package.json` 的 `npm run check`。单测 2/2、全量 **227/227** 通过；真实 Electron 使用独立 `/tmp/mineradio-angle-debug` userData 和 CDP 注入 3 首测试队列，p0 `12°/-18°` 下中心卡片 `rotation.x≈0.219`、`rotation.y≈-0.034`（含既有微小相机/封面跟随），布局返回 `detail.rx≈0.201`、`detail.ry≈-0.294`，无运行时报错。没有改播放、登录、汽水、雨境玻璃水珠或 Windows 死代码。提交时以远端 PR #67 head `e65fcd1c187e82c6ffba5bd4f2a623633fd0f2e3` 为 parent，用 GitHub Git API 新建分支 `codex/shelf-follow-lyric-angle`，创建 tree/commit 后 PATCH ref，`force:false`，不要 `git push`。待用户手测：打开歌词倾角滑条调整 X/Y，确认右侧歌架卡片、歌单详情面板与歌词保持同向；恢复 0° 后确认原有布局。

- **2026-08-02 音域回响右键歌架唤醒 + 删除界面密度（独立 PR 待创建）**：用户反馈预设 10「音域回响」右键唤醒歌架无反应，并要求删除无用的“界面密度”。`public/js/modules/04-shelf/05-card-interactions.js` 的 `contextmenu` 现在调用 `markRenderInteraction('shelf-context', 1200)`（无该函数时回退 `wakeMainLoopFromBackground`），空闲/后台降帧时会立即请求一帧；`public/js/modules/04-shelf/00-layout-hover.js` 新增 `shelfVoxelWorldScale()`，按音域回响远景相机距离把侧栏歌架坐标、卡片尺寸、详情位置和行距同步放大，只影响预设 10，预设 9 雨境玻璃水珠和其他预设保持普通布局。真实 Electron（CDP）验证：p10 `shelfPinnedOpen` 由右键切为 `true`，相机远景下卡片中心约 `x=782–912`，`pickCardAtScreen()` 命中；p9 布局仍为 `sideX=3.18/sideScale=1`。删除 `fx-interface-density-seg` HTML、`FX_CONSOLE_LAYOUT` density 组、`UI_DENSITY_STORE_KEY`/`uiDensityMode` 及全部读写/启动绑定、`ui-density-minimal` CSS。专项 `scripts/test-sonic-sidebar-density.js` 现 9/9，全量 `npm run check` **225/225**。本次仍不改 main、汽水、登录、预设 9 玻璃水珠或 Windows 死代码。建议新 PR 分支 `codex/sonic-shelf-right-wake-density-removal`，parent 使用远端 PR #66 head `537d44e37c9c137f24681ce5b55c8930bcc59724`，GitHub Git API 建 tree/commit/ref，`force:false`，不要 `git push`。待用户在可见窗口手测：p10 右键右侧歌架展开/收起、卡片点击/滚轮、设置页确认不再出现“界面密度”。

- **2026-08-02 音域回响侧栏唤醒与后台过渡修复（独立 PR 待创建）**：用户反馈预设 10「音域回响」下左侧待播放栏、右侧歌架不能唤醒。确认两个根因：① `window.blur` 延迟 60ms 的 `closePlaylistPanelSoft()` 没有在重新聚焦/重新打开时取消，快速切回会被旧计时器关掉；② Electron 页面 hidden/遮挡时 Chromium CSS transition 时间轴会冻结在 0，面板虽然有 `show`/`peek`，仍可能保持 `opacity:0`、`translateX(-404px)`。`public/js/modules/10-shell/02-peek-panels-upload.js` 新增 `playlistPanelWindowBlurTimer`、`cancelPlaylistPanelCloseTimers()`、`refreshPeekPanelsAfterResume()` 和 `resetPeekPanelTransition()`；`public/js/modules/06-lyrics/01-playlist-panel-shell.js` 的 `togglePlaylistPanel()` 在每次打开/关闭前清理旧计时器；`public/js/modules/00-state/08-desktop-render-power.js` 在 focus、visibilitychange、desktop-window 恢复路径调用面板恢复。恢复函数只对活动面板落地一次目标状态，不隐藏核心入口，也不改播放、登录、汽水、雨境玻璃水珠或 Windows 死代码。专项测试 `scripts/test-sonic-sidebar-density.js` 现 8/8；全量 `npm run check` **224/224**。真实 Electron 重载后用 CDP 复现 hidden 页面：恢复前面板 `left:-372`、`opacity:0`，恢复后 `left:32`、`opacity:1`、`transform:matrix(...0,0)`；快速 close/open 后 `class=show` 且 `peekTimer=false`。本批后续以 PR #65 head `cea5260f8731b7396fb9547f474e93daf7a2f6ac` 为 parent，使用 GitHub Git API 创建新分支（建议 `codex/sonic-sidebar-density`）和新 PR，`force:false`，不要追加旧 PR、不要 `git push`。待真实可见窗口再手动确认左右边缘唤醒、3D 歌架 hover 和 4 种窗口尺寸滚动手感。
