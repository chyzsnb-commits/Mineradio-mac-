...
- [ ] **本地曲库 UI 面板**：Windows v2.1.0 有独立本地曲库浏览/管理入口，Mac 本轮只接了导入/恢复/歌词，面板交互（浏览、搜索、移除、按文件夹管理）留待下一批。
