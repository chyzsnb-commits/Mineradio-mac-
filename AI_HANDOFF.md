...
- **当前 Codex 任务链**：PR #27（显卡模式与快速启动）→ PR #29（Mac 真实显卡占用）→ PR #30（主循环真正休眠）→ PR #31（唱歌模式省电）→ PR #32（Mac 安全内存释放）→ PR #33（伴奏/人声双滑块）→ PR #34（最近播放滚动降载与 GPU 文案）均为叠加关系；PR #34 分支 `codex/recent-scroll-gpu-label` 基于 #33，GitHub 设计存档 `620d079`、功能存档 `0723b9e`、审查测试存档 `08a3713`。
