...
- 前端搜索不再提供 QS/汽水模式或汽水搜索请求；音源切换器不再加入汽水，正式版策略保持 `qishuiCatalogEnabled=false`，后端历史目录接口仍由发布策略拦截。
...
- 待办：运行完整 `npm run check`；若仅剩既有 3 个 CI 工作流断言失败，记录并单独处理；推送成功后由仓库主人审查 PR #125。
