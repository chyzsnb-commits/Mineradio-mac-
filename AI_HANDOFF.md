...
- **当前 Codex 任务链**：PR #27（显卡模式与快速启动）→ PR #29（Mac 真实显卡占用）→ PR #30（主循环真正休眠）→ PR #31（唱歌模式省电）→ PR #32（Mac 安全内存释放）→ PR #33（伴奏/人声双滑块）→ PR #34（最近播放滚动降载与 GPU 文案）→ PR #36（播放定时器降载）→ PR #37（本地 AI 分轨）→ PR #38（AI 提速与实时精准度）→ PR #39（软件 Logo）→ PR #40（歌词选项切换降卡）→ `codex/audio-proxy-disconnect`（音频上游断线保护）均为叠加关系；最新分支基于 PR #40。
...
17. **音频上游断线保护**（`codex/audio-proxy-disconnect`）：上游音频传输中断且响应头已经发出时，只关闭当前损坏连接，不再重复发送 500 响应，避免 `ERR_HTTP_HEADERS_SENT` 让本地服务退出。
...
- 分支：`codex/audio-proxy-disconnect`；基于 PR #40。
