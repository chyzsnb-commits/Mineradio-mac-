...
- **当前 Codex 任务链**：PR #27（显卡模式与快速启动）→ PR #29（Mac 真实显卡占用）→ PR #30（主循环真正休眠）→ PR #31（唱歌模式省电）→ PR #32（Mac 安全内存释放）均为叠加关系；PR #32 分支 `codex/mac-memory-safe-auto` 基于 #31，设计存档 `01e4dea`、功能存档 `669348e`，实现 Mac 播放中分级自动清理。
