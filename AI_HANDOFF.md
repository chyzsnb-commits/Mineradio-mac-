...
- **当前 Codex 任务链**：PR #27（显卡模式与快速启动）→ PR #29（Mac 真实显卡占用）→ PR #30（主循环真正休眠）→ PR #31（唱歌模式省电）→ PR #32（Mac 安全内存释放）→ PR #33（伴奏/人声双滑块）→ PR #34（最近播放滚动降载与 GPU 文案）→ PR #36（播放定时器降载）→ PR #37（本地 AI 分轨）→ PR #38（AI 提速与实时精准度）→ PR #39（软件 Logo）→ PR #40（歌词选项切换降卡）→ PR #41（音频上游断线保护）→ `codex/playback-skip-reset`（整队不可播保护）均为叠加关系；最新分支基于 PR #41。
...
18. **整队不可播计数修复**（`codex/playback-skip-reset`）：拿到音频 URL 不再提前清零连续跳过计数；只有当前切歌任务确认本地或网络歌曲真正播放后才清零，避免无效 URL 让整队保护失效。
...
- 分支：`codex/playback-skip-reset`；基于 PR #41。
