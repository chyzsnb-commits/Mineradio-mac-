...
- 验证：`npm run check` 共 100 项全部通过；Electron 使用独立资料和假媒体参数正常启动；未签名 arm64 目录包通过，`app.asar` 已确认包含最终线程、电源和瞬态保护代码。
