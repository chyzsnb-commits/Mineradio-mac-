...
- 修复 P10 音域回响一级歌架投到右侧远景：图二证明 P10 不能直接复用普通预设的局部位置/比例。恢复图一所需的 P10 世界构图补偿，包括横向回收、完整纵向堆叠、`0.82` 局部缩放和轻微稳定根组姿态；普通预设不受影响。相机前方安全锚点、稳定世界比例、原生镜头保护、右键仅一级、歌词让位和冷色玻璃卡面保持。新增图一构图范围回归测试；专项 `21/21` 通过。[来源: `public/js/modules/04-shelf/00-layout-hover.js`、`scripts/test-p10-voxel-interactions.js`]
