...
- 彻底修复 3D 歌架与歌词角度不同步及歌单“乌漆麻黑”：此前歌架只把 `lyricTiltX/Y` 叠到卡片局部欧拉角，而歌词实际使用相机/封面基准乘倾角后的最终四元数，因此在不同镜头和预设下仍会歪。现在右侧歌架根组与歌单详情根组直接复用 `stageLyrics.group.quaternion`，卡片只保留自身扇形展开与用户独立歌架微调；歌单卡片、详情面板和歌曲行由近乎不透明的纯黑底改为歌词色板驱动的半透明玻璃底，详情展开时背景卡片保持可辨。新增 4 项 `scripts/test-shelf-lyric-angle.js` 回归；真实 Electron CDP 验证歌词与侧栏根组四元数一致、详情组保持同一基准且材质像素不再纯黑；`npm run check` **229/229** 通过。不改播放、登录、汽水或预设 9 雨境玻璃水珠。
