...
- 修复 P10 音域回响一级歌架与其他预设不一致：移除 P10 专属的位置、间距、进场位移、`0.82` 缩放和附加根组倾角，直接复用普通预设的竖向歌架布局与指针姿态。P10 的相机前方安全锚点、稳定世界比例、原生镜头保护、右键仅一级、歌词让位及冷色玻璃卡面仍保持。新增“P10 布局对象与普通预设完全相同”的回归测试；专项 `21/21`、完整 `npm run check` `271/271` 通过，命令行 Electron 已启动。[来源: `public/js/modules/04-shelf/00-layout-hover.js`、`scripts/test-p10-voxel-interactions.js`]
