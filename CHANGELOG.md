...
- 新增中英文界面切换：顶部语言按钮可在中文与 English（英文）之间切换，语言偏好保存到本机并覆盖主页、搜索、播放器、队列、视觉控制台、登录、壁纸控制等界面文案；歌曲名、歌手名、歌词和用户输入保持原文。新增动态节点翻译与回归测试，未引入高频字符监听，避免影响渲染性能。
...
- “3D / 手势”新增本地手部模型库：保留经典流光，加入晶体、星尘与 WebXR Generic Hand 实体模型，并支持用户导入自包含 GLB / glTF、调整大小、选择和删除。模型只保存在本机 IndexedDB；外链资源、超限文件、过高面数或异常场景会在渲染前拒绝，摄像头帧不上传。
- 手模导入预检补齐 GLB 唯一 JSON/BIN、标准顺序、buffer 唯一来源、场景/相机上限、单父节点树和实例化绘制预算；恶意重复 chunk、共享 DAG、海量场景或重复网格会在进入 GLTFLoader 前被拒绝。手势关闭后释放透明 WebGL 上下文，未开启手势时只保存模型选择、不占用 GPU。
- “词境穿行”编排从 12 套扩为 16 套，新增景深接力、近景回卷、折页展开和坠落回弹；四类动画复用原有五张歌词卡和单一主循环，不增加纹理、draw call 或独立 RAF。顺序换句、反向 seek、歌词关闭与系统“减少动态”均保持连续，不再瞬移或闪回隐藏歌词。
- 验证：手模/手势安全专项 23/23、词境穿行专项 26/26，完整 `npm run check` **250/250**。隔离 Electron 使用合成歌词逐帧验收四类新动画；隔离 Chrome 假摄像头下，透视与手势引用同一条 `1280×720` MediaStream，Worker 保持比例输入 `320×180`，GPU 推理约 20 次/秒；真实摄像头、真实资料目录和登录态均未用于自动化。
- 视觉控制台“背景（所有预设通用）”新增全局“透视模式”：摄像头画面作为实时底图，默认舞台的歌词与封面、粒子预设、P10“音域回响”和 P11“词境穿行”继续显示在前景；关闭后恢复此前选中的图片、视频或颜色背景，不需要重启。
- 背景区移除 6 枚重复且观感不统一的快捷色块，保留主调色板和“默认”入口，把原占位用于透视模式；摄像头只在视频首帧可用后接管背景，窗口隐藏、最小化、遮挡、进入壁纸模式或启动页时暂停采集，回到前台按原开关恢复。
- 手势识别与透视模式改用同一个摄像头管理器：以 `gesture` / `perspective` owner 共享单条 MediaStream，透视加入时提升画质、退出后按剩余手势需求降档，最后一个 owner 释放才真正停流；先走 macOS 原生权限检查，拒绝、缺设备和无首帧分别提示，不重复申请摄像头。
- 从 PR #113 的实际实现祖先中选择性融合 Wallpaper Engine 局域网素材库：只接入经过 ping 验证的 Windows 服务、图片/视频导入和 Scene 预览/导出流程，并限制可信渲染页调用；列表和媒体改为 4MB / 256MB 流式限额，视频不再整段复制过 IPC，未验证来源和错误 MIME 会被拒绝；没有把 PR #113 整条移植线覆盖到当前稳定 App，也没有恢复已删除的封面裁切界面。
- 汽水恢复为无需登录的“公开目录 + 歌词”能力：搜索结果永远标记为不可直放，只能通过严格匹配交给现有音源自动换源；发布策略继续禁止汽水登录、Cookie/Token/本地客户端会话读取、受保护媒体处理与解密链路。
- 验证：透视模式 6/6、共享摄像头 4/4、Wallpaper 9/9、汽水目录与公开策略 11/11 专项检查通过，完整 `npm run check` 为 **228/228**。隔离 Electron 使用 Chromium 假摄像头验收，全程未调用真实摄像头：透视视频为 `1280×720` 且 `readyState=4`，P10/P11 下均保持摄像头底图与透明 WebGL；手势与透视同时开启时只有 1 次采集，Wallpaper 面板可进入局域网发现，汽水真实目录搜索返回封面且 `playable=false`。
- 新增视觉预设“词境穿行”：按参考视频复刻深空封面与景深歌词剧场；换句时新句整组从深处推进到焦面，旧句越过焦面放大、失焦并短暂重叠。封面使用原片可见的命中互动：约 8% 弹性放大、随指针轻倾并在移开后带轻微回弹，不把交互扩散到歌词。
- “词境穿行”的歌单入口与“音域回响”一致，复用视觉控制台里的普通歌单，不显示或接管 3D 歌单架；两个预设直接互切时歌单保持正确宿主，退出后恢复原位置。
- 词境穿行遵守歌词总开关；无封面歌曲不再残留上一首封面；快速拖动进度条时最多约每 90ms 上传一张当前歌词纹理，松手后再补齐景深层，避免连续切歌词造成卡顿。
- 词境穿行现在实时读取歌词字号、字体、字重、字距与翻译设置；该预设固定使用五层景深，因此“歌词行数”按钮与滑杆会禁用且退出预设后恢复。新增“跟唱明暗”，原文唱过部分提亮、未唱部分压暗，译文不参与扫亮。
- 动态页新增“360° 词境漫游”大按钮：鼠标拖动旋转，滚轮缩放；开启摄像头后复用 Emily 的双手距离缩放与连线旋转。关闭或重开会回正，不继承旧角度。
- 词境穿行的镜头编排由 3 种扩展为 12 套侧入、景深、上升、斜切、环绕、横扫、漂浮与蓄力穿越；跨歌曲改为旧场退离、星尘桥接、新封面与歌词到齐后入场，慢歌词和自动换源期间不再空等、回亮或硬切。
- 修复 macOS 手势识别点击后不弹摄像头授权：完整打包为主 App 与 Helper 声明摄像头/麦克风 entitlement，用户首次开启时主动调用系统授权；拒绝、设备占用、无画面和识别引擎错误分别给出准确提示，不再全部误报为“需要摄像头权限”。
- 修复失败移植版遗留的“歌词关闭”状态不易察觉：底栏“词”按钮现在同步高亮与 `aria-pressed`，用户切换后立即保存；本机安装时将被污染的歌词状态定点恢复为开启、自动配色与正常发光。
- 以已恢复的稳定 App 对应源码为基线做第一批选择性融合：只保留原版“音域回响”，不引入移植版的其他音域模式或页面。
- “悬浮方块”新增 100%–200% 尺寸滑块；100% 保持原版大小，200% 接近移植版大方块观感。实现只缩放现有实例，不增加方块数量或 draw call，并随体素设置保存。
- 纠正音域回响双手缩放语义：双手张开/收拢直接缩放音域内容（55%–190%），不再复用滚轮相机；撤销全局目标/显示相机缓动，鼠标长按拖动恢复原版即时响应；手势缩放期间持续唤醒渲染，避免空闲 2FPS 时跳变。
- 修复全屏播放器遇到长标题时红心和“+”掉到下一行：歌曲区固定单行，标题可靠省略，2048 与 1440 宽度实测通过。
- 删除背景区域的“封面 / 裁切 / 清除”三个按钮及背景裁切专用弹窗；图片/视频背景、位置/缩放滑条与歌曲封面裁切继续保留。
...
- 公开包仍不包含汽水登录、直接播放与音频解密器；禁用本地 Cookie 数据库读取、Cookie/Token 粘贴、模拟官方客户端扫码和受保护音频处理，只保留不需要账号的公开目录/歌词查询以及严格匹配后的现有音源换源。
...
- 旧歌单或播放队列里残留的汽水直连曲目会明确标记为不可直接播放，不再错误套用网易接口；登录页不提供汽水账号入口，搜索音源栏仅显示目录查询用的 `QS`，命中后仍必须走严格匹配换源。
