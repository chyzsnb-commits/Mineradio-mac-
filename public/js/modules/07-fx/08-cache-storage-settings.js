...
function applyMineradioCacheSettings(snapshot) {
  ...
  var count = Number(snapshot.lyricsCount) || 0;
  setMineradioCacheStorageText('cache-storage-total', '已占用 ' + formatMineradioCacheBytes(snapshot.lyricsBytes) + (count ? ' · ' + count + ' 条' : ''));
  ...
  setMineradioCacheStorageText('cache-storage-chromium-path', 'Mac 不迁移 Chromium 缓存（保护登录态）');
  setMineradioCacheStorageText('cache-storage-chromium-size', '—');
  setMineradioCacheStorageText('cache-storage-beatmaps-path', '—');
  setMineradioCacheStorageText('cache-storage-beatmaps-size', '—');
  setMineradioCacheStorageText('cache-storage-wallpaper-path', '—');
  setMineradioCacheStorageText('cache-storage-wallpaper-size', '—');
  ...
  setMineradioCacheStorageText('cache-storage-note', '歌词磁盘缓存（userData/cache/lyrics）· 单条 ≤1MB、总量 ≤96MB 自动淘汰');
}
