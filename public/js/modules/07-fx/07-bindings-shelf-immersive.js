function bindFxPanel() {
  ...
  ids.forEach(function (pair) {
    ...
    el.addEventListener('input', function () {
      ...
      if (pair[1] === 'playlistPanelOpenDuration') fx.playlistPanelOpenDuration = clampRange(fx.playlistPanelOpenDuration, 0.12, 0.90);
      if (pair[1] === 'playlistPanelCloseDuration') fx.playlistPanelCloseDuration = clampRange(fx.playlistPanelCloseDuration, 0.08, 0.72);
  ...
  ['ui-accent-picker', 'visual-tint-picker', 'home-accent-picker', 'home-icon-picker', 'visual-icon-picker', 'bg-color-picker', 'shelf-accent-picker', 'lyric-color-picker', 'lyric-highlight-picker', 'lyric-glow-picker', 'vox-color-pick', 'vox-ripple-pick'].forEach(function (id) {
    bindColorLabPicker(document.getElementById(id));   // 体素两个取色块必须进名单: 原生 input[type=color] 在本 app 里被颜色实验室体系替代, 未绑定=点击死区
  });
  bindColorLabRows();
  ...
  var renderScaleEl = document.getElementById('fx-renderscale');
  if (renderScaleEl) {
    renderScaleEl.value = getRenderScale();
    var renderScaleOut = renderScaleEl.parentElement.querySelector('output');
    if (renderScaleOut) renderScaleOut.textContent = renderScaleResLabel();
    renderScaleEl.addEventListener('input', function () {
      setRenderScale(renderScaleEl.value);
      if (renderScaleOut) renderScaleOut.textContent = renderScaleResLabel();
    });
  }
...
function toggleFx(key) {
  ...
  if (key === 'albumBackgroundMouseBind' || key === 'wallpaperMouseParallax') updateFxInputs();
  ...
  syncFxUniforms();
  if (key === 'lyricCameraLock' || key === 'lyricGlow' || key === 'lyricGlowBeat' || key === 'lyricGlowParticles' || key === 'lyricVerticalFloat' || key === 'backgroundStarRiver' || key === 'lyricPauseHold' || key === 'bloom' || key === 'edge' || key === 'cinema' || key === 'aiDepth' || key === 'desktopLyrics' || key === 'desktopLyricsClickThrough' || key === 'desktopLyricsCinema' || key === 'desktopLyricsHighlight' || key === 'wallpaperMode' || key === 'shelfShowPodcasts' || key === 'shelfMergeCollections' || key === 'liveBackgroundKeep' || key === 'memoryAutoTrimApp' || key === 'memoryAutoTrimOnBackground' || key === 'memoryAutoSystemTrim' || key === 'memorySystemAutoElevate' || key === 'albumBackgroundMouseBind' || key === 'wallpaperMouseParallax') saveLyricLayout({ user: true, reason: key });
  ...
  if (key === 'wallpaperMouseParallax' && typeof updateCustomBackgroundMouseParallax === 'function') updateCustomBackgroundMouseParallax();
  ...
  if (key === 'wallpaperMouseParallax') showToast(fx.wallpaperMouseParallax ? '壁纸鼠标视差已开启' : '壁纸鼠标视差已关闭');
...
function setShelfMode(m, opts) {
  opts = opts || {};
  m = 'side';  // 底部横向 3D 歌单(stage/both)已删,3D 歌架只以右侧 side 呈现;左侧 2D 面板独立于此
  var prevShelf = fx.shelf;
  var prevPinned = fx.shelfPinnedOpen;
  var prevPresence = fx.shelfPresence;
  fx.shelf = m;
  if (m === 'stage' || m === 'both') shelfHardHidden = false;   // 重新选带 3D 架的模式 -> 取消硬隐藏
  // both = stage 的 3D 架 + side 的 2D 侧边歌单同时显示; 3D 架内部仍走 stage 呈现。
  var managerMode = (m === 'both') ? 'stage' : m;
  var stageLike = (m === 'stage' || m === 'both');
  ...
  if (shelfManager) shelfManager.setMode(managerMode);
  // 舞台 / 都保留: 顶部搜索、底部控件让位
  var searchArea = document.getElementById('search-area');
  var bottomBar = document.getElementById('bottom-bar');
  if (searchArea) searchArea.classList.toggle('stage-mode', stageLike);
  if (bottomBar) bottomBar.classList.toggle('stage-mode', stageLike);
  // 都保留: 额外常显 2D 侧边歌单; 离开 both 时收起(不动用户独立的「常开歌单」pin)
  if (typeof togglePlaylistPanel === 'function') {
    if (m === 'both') togglePlaylistPanel(true);
    else if (prevShelf === 'both' && !playlistPanelPinned) togglePlaylistPanel(false);
  }
...
function toggleShelfFromControls() {
  ...
  if (shelfHardHidden) {
    ...
    shelfHardHidden = true;
    if (typeof safeShelfCloseContent === 'function') safeShelfCloseContent('shelf-controls-toggle');
    setShelfPinnedOpen(false, true);
    shelfHoverCue.target = 0; shelfHoverCue.value = 0; shelfHoverCue.zoneActive = false; shelfHoverCue.guide = false;
    if (typeof setFocusZone === 'function') setFocusZone(null, true);
  }
  syncShelfToggleBtn();
}
function syncShelfToggleBtn() {
  var btn = document.getElementById('shelf-toggle-btn');
  if (!btn) return;
  btn.classList.toggle('active', !shelfHardHidden);
  btn.title = '右侧 3D 歌单架 显示/隐藏';
}
...
function setShelfPresence(mode) {
  ...
  updateShelfControlUi();
  if (shelfManager && shelfManager.setMode) shelfManager.setMode('side');   // 只保留右侧 3D 歌架
