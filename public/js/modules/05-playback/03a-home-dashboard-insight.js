...
// ============================================================
//  Windows v2.1.0 对齐: 首页洞察 dock + 平台推荐中心
//  (macOS 版: 不迁移 Windows 的 MP4 视频 Hero,保留 Mac 最近播放/天气/歌单入口)
// ============================================================
...
function scheduleHomeDashboardRefresh() {
  ...
  homeDashboardRefreshTimer = setTimeout(function () {
    homeDashboardRefreshTimer = null;
    if (!emptyHomeActive || document.hidden) return;
    if (typeof homeDashboardUpdateReviewClock === 'function') homeDashboardUpdateReviewClock();
...
// 在既有 renderHomeDiscover 之后补渲染洞察 dock,保留 Mac 原有首页内容
var homeDashboardInsightBaseRenderHomeDiscover = typeof renderHomeDiscover === 'function' ? renderHomeDiscover : null;
if (homeDashboardInsightBaseRenderHomeDiscover) {
  renderHomeDiscover = function () {
    var result = homeDashboardInsightBaseRenderHomeDiscover.apply(this, arguments);
    renderHomeDashboard();
    return result;
  };
}
