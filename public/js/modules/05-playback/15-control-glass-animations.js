var controlGlassState = { key: '', searchBoxKey: '', searchPillKey: '', accountPillKey: '' };
// P4:glass-lite(过热/eco 降级)时暂停位移贴图更新 —— 此时 CSS 已把常驻玻璃换成普通 blur,SVG 贴图不再被引用,
// 继续 getBoundingClientRect + 生成 SVG data URI 纯属浪费。恢复时补刷一次,追平降级期间跳过的尺寸变化。
var _controlGlassUpdatesPaused = false;
function setControlGlassDisplacementPaused(paused) {
  paused = !!paused;
  if (paused === _controlGlassUpdatesPaused) return;
  _controlGlassUpdatesPaused = paused;
  if (!paused) {
...
function normalizeControlGlassChromaticOffset(value) {
  ...
  return clampRange(n, 0, 140);   // 上游下限 30;放到 0=彻底关闭色散(无风扇 mac 上 90 档 GPU 近乎翻倍,默认 0)
}
...
function updateControlGlassDisplacementMap() {
  if (_controlGlassUpdatesPaused) return true;   // P4:glass-lite 降级期间贴图不被引用,跳过重算
...
function updateSearchBoxGlassDisplacementMap() {
  if (_controlGlassUpdatesPaused) return true;   // P4:同上,返 true 避免误判 fallback
...
function updateSearchPillGlassDisplacementMap() {
  if (_controlGlassUpdatesPaused) return true;   // P4:glass-lite 降级期间跳过
...
function updateAccountPillGlassDisplacementMap() {
  if (_controlGlassUpdatesPaused) return;   // P4:glass-lite 降级期间跳过
