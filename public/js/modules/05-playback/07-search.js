...
function updateSearchModeTabs() {
  ...
  if (ytmusicBtn) {
    ytmusicBtn.classList.toggle('active', searchMode === 'ytmusic');
    ytmusicBtn.setAttribute('aria-selected', searchMode === 'ytmusic' ? 'true' : 'false');
  }
  if (podcastBtn) {
  ...
  if ($input && searchMode === 'ytmusic') $input.placeholder = '搜索 YouTube Music...';
  requestAnimationFrame(updateSearchPillGlassDisplacementMap);
}
function setSearchMode(mode) {
  mode = (mode === 'podcast' || mode === 'netease' || mode === 'qq' || mode === 'kugou' || mode === 'spotify' || mode === 'ytmusic') ? mode : 'song';
...
function songProviderKey(song) {
  ...
  if (song && (song.provider === 'ytmusic' || song.source === 'ytmusic')) return 'ytmusic';
...
function songSourceTagHtml(song, opts) {
  ...
  var key = /^(netease|qq|kugou|qishui|spotify|ytmusic)$/.test(String(rawKey || '')) ? String(rawKey) : songProviderKey(song);
  var label = key === 'qq' ? 'QQ' : (key === 'kugou' ? 'KG' : (key === 'qishui' ? 'QS' : (key === 'spotify' ? 'SP' : (key === 'ytmusic' ? 'YT' : 'NE'))));
  if (opts.switcher) {
...
var MUSIC_SEARCH_PROVIDER_ORDER = ['netease', 'qq', 'kugou', 'spotify'];
...
function searchModeProvider(mode) {
  return mode === 'netease' || mode === 'qq' || mode === 'kugou' || mode === 'spotify' ? mode : '';
}
...
function searchProviderLoginNotice(mode) {
  ...
  if (specific) {
    ...
    return '先登录 ' + (meta.label || specific) + ' 后再搜索该平台';
  }
  return '先登录至少一个音乐平台后再搜索';
}
...
function searchPopularityScore(song, sourceIndex) {
  ...
  var rank = Number(song && (song.searchRank || song.rank || song.spotifyRank || song.kugouRank || song.qishuiRank));
  var score = 0;
  if (isFinite(hot) && hot > 0) {
    score += hot <= 100 ? Math.min(22, hot / 5) : Math.min(26, Math.log(hot + 1) * 2.2);
  }
  if (isFinite(rank) && rank >= 0) {
    score += Math.max(0, 18 - rank * 1.6);
  } else {
    score += Math.max(0, 10 - (sourceIndex || 0) * 1.2);
  }
  return score;
}
function searchCanonicalSongKey(song) {
  var title = simpleSearchNorm(song && song.name);
  var artists = sourceSwitchArtistParts(song).slice(0, 3);
...
function scoreSongSearchResult(song, q, sourceIndex) {
  ...
  var raw = String(((song && song.name) || '') + ' ' + ((song && song.artist) || '') + ' ' + ((song && song.album) || '')).toLowerCase();
  var qAsksDerivative = /(live|现场|翻唱|cover|伴奏|instrumental|remix|dj|片段|demo|女声|男声|karaoke|氛围|浴室|节奏|进行曲|加速|慢速|变速|串烧)/i.test(String(q || ''));
  ...
  var score = 0;
  if (sourceSwitchSongHasBlockedArtist(song)) score -= 999;
  if (name === nq) score += 90;
  ...
  if (provider === 'netease') score += 5;
  if (provider === 'qq') score += searchIntentPrefersQQ(q) ? 48 : 5;
  if (provider === 'kugou') score += 2;
  if (provider === 'qishui') score -= 2;
  if (provider === 'spotify') score -= 2;
...
function mergeSongSearchResults(neteaseSongs, qqSongs, kugouSongs, qishuiSongs, spotifySongs, limit, q) {
  ...
  function push(song, sourceIndex) {
    ...
    if (sourceSwitchSongHasBlockedArtist(song)) return;
...
async function fetchMusicSearchResults(q, mode) {
  ...
  if (mode === 'ytmusic') {
    var ytOnly = await apiJson('/api/ytmusic/search?keywords=' + encodeURIComponent(q) + '&limit=20');
  ...
  var limitByProvider = { netease: 18, qq: 12, kugou: 12, spotify: 12 };
  var result = await Promise.allSettled(providers.map(function (provider) {
    return apiJson(searchProviderUrl(provider, q, limitByProvider[provider] || 12));
  }));
  var neteaseSongs = [];
  var qqSongs = [];
  var kugouSongs = [];
  ...
  providers.forEach(function (provider, index) {
    ...
    if (provider === 'netease') neteaseSongs = songs;
    if (provider === 'qq') qqSongs = songs;
    if (provider === 'kugou') kugouSongs = songs;
    if (provider === 'spotify') spotifySongs = songs;
    if (value.message && !songs.length && !searchProviderNotice) searchProviderNotice = value.message;
  });
  return mergeSongSearchResults(neteaseSongs, qqSongs, kugouSongs, [], spotifySongs, 18, q);
}
