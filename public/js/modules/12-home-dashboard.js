...
function homeDashboardApplyHeroThemeColor(color) {
  ...
  if (typeof fx !== 'undefined' && fx) {
    ...
    if (value) value.textContent = '背景取色';
  }
}

function homeDashboardTryUpdateHeroTheme(mediaEl, attempt, opts) {
  opts = opts || {};
  ...
  if (accent) {
    homeDashboardApplyHeroThemeColor(accent);
    return;
  }
  if ((attempt || 0) >= 4) return;
  setTimeout(function () {
    homeDashboardTryUpdateHeroTheme(mediaEl, (attempt || 0) + 1, opts);
  }, 160 * ((attempt || 0) + 1));
}

function homeDashboardUpdateHeroTheme(mediaEl, opts) {
  homeDashboardTryUpdateHeroTheme(mediaEl, 0, opts);
}

function homeDashboardApplyThemeFromBlob(blob, kind) {
  if (!blob || !homeDashboardHeroAccentAutoEnabled()) return Promise.resolve();
  return new Promise(function (resolve) {
    var url = '';
    try { url = URL.createObjectURL(blob); } catch (_error) { resolve(); return; }
    if (kind === 'image') {
      var img = new Image();
      var done = function () {
        if (url) {
      ...
      img.onload = function () {
        if (typeof img.decode === 'function') {
          img.decode().then(function () {
            homeDashboardUpdateHeroTheme(img, { force: true });
            done();
          }).catch(function () {
            homeDashboardUpdateHeroTheme(img, { force: true });
            done();
    ...
    video.addEventListener('loadeddata', function () {
      try { video.currentTime = Math.min(0.12, Math.max(0, (video.duration || 0.5) * 0.08)); } catch (_error) {
        homeDashboardUpdateHeroTheme(video, { force: true });
        finish();
      }
    }, { once: true });
...
function homeDashboardScheduleHeroThemeUpdate(mediaEl, opts) {
  if (!mediaEl || !homeDashboardHeroAccentAutoEnabled()) return;
  ...
  homeDashboardHeroThemeRaf = requestAnimationFrame(function () {
    ...
    homeDashboardUpdateHeroTheme(mediaEl, opts);
  });
}
...
function homeDashboardBindHeroThemeMedia(mediaEl, kind) {
  ...
  if (kind === 'image') {
    ...
    if (typeof mediaEl.decode === 'function') {
      mediaEl.decode().then(run).catch(run);
    }
    setTimeout(run, 220);
    setTimeout(run, 520);
    return;
  }
  if (mediaEl.readyState >= 2 && mediaEl.videoWidth) run();
  mediaEl.addEventListener('loadeddata', run, { once: true });
  mediaEl.addEventListener('seeked', run, { once: true });
  mediaEl.addEventListener('playing', run, { once: true });
  setTimeout(run, 260);
  setTimeout(run, 720);
}
...
async function homeDashboardAttachVideo() {
  ...
  try {
    ...
    homeDashboardApplyThemeFromBlob(blob, kind);
...
async function handleHomeDashboardVideoFile(file) {
  ...
  try {
    if (typeof fx !== 'undefined' && fx) fx.homeAccentMode = 'auto';
    await homeDashboardApplyThemeFromBlob(file, kind);
