...
if (typeof loadRainResonanceSettings === 'function') loadRainResonanceSettings(); // 恢复云瀑共振独立控件
bindFxPanel();
...
var persistentLocalRestorePromise = Promise.resolve(restorePersistedLocalLibrary()).then(function (persistentRestored) {
  if (persistentRestored) restoredPlaybackAtStartup = true;
  return !!persistentRestored;
...
var startupReadyPromise = Promise.all([startupLoginStatusPromise, persistentLocalRestorePromise]);
...
if (startupReadyPromise && startupReadyPromise.then) {
  startupReadyPromise.then(function () {
