...
var playlistPanelWindowBlurTimer = null;
var PLAYLIST_PANEL_MOTION_MS = 360;
var PLAYLIST_PANEL_OPEN_ANIM_COOLDOWN = 520;
function cancelPlaylistPanelCloseTimers() {
  if (peekTimers.pl) {
    clearTimeout(peekTimers.pl);
    peekTimers.pl = null;
  }
  if (playlistPanelWindowBlurTimer) {
    clearTimeout(playlistPanelWindowBlurTimer);
    playlistPanelWindowBlurTimer = null;
  }
}
function resetPeekPanelTransition(el) {
  if (!el) return;
  var active = el.classList.contains('show') || el.classList.contains('peek');
  if (!active) return;
  // 后台/遮挡时 Chromium 会冻结 CSS transition 时间轴。恢复窗口时把目标状态
  // 直接落地一次，避免元素带着 opacity:0 和旧的 translateX 停在屏幕外。
  var transition = el.style.getPropertyValue('transition');
  var transitionPriority = el.style.getPropertyPriority('transition');
  ...
  if (transition) el.style.setProperty('transition', transition, transitionPriority);
  else el.style.removeProperty('transition');
}
function refreshPeekPanelsAfterResume() {
  cancelPlaylistPanelCloseTimers();
  var playlist = document.getElementById('playlist-panel');
  if (playlist) {
    playlist.classList.remove('playlist-panel-closing');
    resetPeekPanelTransition(playlist);
  }
  resetPeekPanelTransition(document.getElementById('search-area'));
  resetPeekPanelTransition(document.getElementById('fx-panel'));
}
function playlistPanelMotionMs(type) {
  var key = type === 'close' ? 'playlistPanelCloseDuration' : 'playlistPanelOpenDuration';
  var fallback = type === 'close' ? 0.26 : 0.34;
  var value = fx && isFinite(fx[key]) ? Number(fx[key]) : fallback;
  return Math.round(clampRange(value, type === 'close' ? 0.08 : 0.12, type === 'close' ? 0.72 : 0.90) * 1000);
}
...
function setPeek(el, on, key) {
  ...
  if (on) {
    ...
    if (key === 'pl') cancelPlaylistPanelCloseTimers();
    ...
    if (key === 'pl' && isPlaylistPanelInMotion(el)) {
      var remaining = Math.max(60, Math.min(PEEK_HIDE_DELAY + 140, (el.__playlistMotionUntil || performance.now()) - performance.now()));
      peekTimers[key] = setTimeout(function () {
        if (!playlistPanelPinned) {
...
function playlistPanelFocusPadding() {
  return isSecondaryLeftDisplaySeamGuardActive() ? 48 : 72;
}
function isPlaylistPanelActiveState(panel) {
  return !!(panel && (panel.classList.contains('peek') || panel.classList.contains('show') || isPlaylistPanelInMotion(panel)));
}
...
function shouldClosePlaylistPanelFromPointer(ppOn, ex, ppRect) {
  ...
  var panel = document.getElementById('playlist-panel');
  var targetRect = playlistPanelTargetRect(panel, ppRect);
  if (isPlaylistPanelInMotion(panel) && ex < targetRect.right + playlistPanelFocusPadding()) return false;
...
function isPlaylistPanelFocusActive(inTrigger, inPanel, pp, ex, ppRect) {
  if (isSecondaryLeftDisplaySeamGuardActive() && ex < SECONDARY_PLAYLIST_SEAM_CLOSE_X) return false;
  var targetRect = playlistPanelTargetRect(pp, ppRect);
  return inTrigger || inPanel || (isPlaylistPanelActiveState(pp) && ex < targetRect.right + playlistPanelFocusPadding());
}
...
window.addEventListener('blur', function () {
  ...
  if (playlistPanelWindowBlurTimer) clearTimeout(playlistPanelWindowBlurTimer);
  playlistPanelWindowBlurTimer = setTimeout(function () {
    playlistPanelWindowBlurTimer = null;
    closePlaylistPanelSoft('window-blur');
  }, 60);
});
window.addEventListener('focus', refreshPeekPanelsAfterResume);
document.addEventListener('visibilitychange', function () {
  if (document.visibilityState === 'visible') refreshPeekPanelsAfterResume();
});
window.addEventListener('mousemove', function (e) {
  ...
  if (immersiveMode) {
    ...
    var queueFocusImm = isPlaylistPanelFocusActive(inQueueTriggerImm, inQueuePanelImm || inQueueBridgeImm, pp, ex, ppRectImm);
  ...
  var newFocus = null;
  var queueFocusActive = isPlaylistPanelFocusActive(inQueueTrigger, inQueuePanel || inQueueBridge, pp, ex, ppRect);
