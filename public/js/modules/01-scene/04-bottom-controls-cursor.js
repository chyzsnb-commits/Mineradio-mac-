...
function revealBottomControls(delay, force) {
  if (document.body.classList.contains('home-controls-locked')) return;
  var bar = document.getElementById('bottom-bar');
  // force=true:用户主动展开播放篮等,即使右侧 3D 歌单架开着也要露出底栏(否则 popover 有 .show 但 opacity 0 打不开)
  if (!force && isBottomControlsSuppressedForShelf()) return;
