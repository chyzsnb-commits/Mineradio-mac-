...
function clearCenteredViewOffsets() {
  ...
  if (typeof clearVoxelGestureUniforms === 'function') clearVoxelGestureUniforms();
}
...
function shouldUseSkullSafeShelfCamera() {
  ...
  // 不再用专用的近中心大卡摆法(那套会盖脸)。返回 false = 歌架的布局/姿态/详情全走普通预设分支。
  return false;
}
...
function recenterCamera() {
  // 音域回响:回正归位到原作默认机位(整片俯瞰);K 键与双击都走这里
  if (typeof voxelCityActive === 'function' && voxelCityActive() && typeof voxRecenterCamera === 'function') voxRecenterCamera();
