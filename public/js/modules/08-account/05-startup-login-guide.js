...
function maybeRunStartupLoginGuide(source) {
  ...
  // 任一平台(网易云/QQ/酷狗)已登录都不弹引导——原来只看网易云,QQ 用户每次启动都被弹(用户实测)
  if (!loginStatusChecked || loginStatusCheckFailed || loginStatus.loggedIn || (typeof hasAnyPlatformLogin === 'function' && hasAnyPlatformLogin()) || playing) return;
  ...
  setTimeout(function () {
    if (loginStatus.loggedIn || (typeof hasAnyPlatformLogin === 'function' && hasAnyPlatformLogin()) || playing || immersiveMode || document.body.classList.contains('splash-active')) return;
