...
function bindModalBackdropClose() {
  [
    ...
    ['gpu-mode-restart-modal', dismissGpuModeRestartPrompt]
...
function providerAvatarSrc(provider, status) {
  ...
  var fill = provider === 'qq' ? '#bfd66b' : (provider === 'kugou' ? '#ff6b6b' : (provider === 'qishui' ? '#45d68f' : (provider === 'spotify' ? '#1ed760' : '#d95b67')));
  var bg = provider === 'qq' ? '#11150b' : (provider === 'kugou' ? '#1a0b0b' : (provider === 'qishui' ? '#071a12' : (provider === 'spotify' ? '#06140a' : '#180b0f')));
