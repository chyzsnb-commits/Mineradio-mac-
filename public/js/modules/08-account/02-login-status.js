...
function normalizeSpotifyLoginStatus(info) {
  var fallback = { provider: 'spotify', loggedIn: false, configured: false, oauthConfigured: false, oauthMissing: [], preview: false, nickname: 'Spotify', userId: '', avatar: '', product: '', vipType: 0, vipLevel: 'none', isVip: false, isSvip: false, stale: false, playbackKeyReady: false, playbackMode: 'recommend-match', tokenConfigured: false, tokenFileExists: false, credentialsFileExists: false, localConfigMissing: false, searchReady: false };
  ...
  return Object.assign({}, fallback, info || {}, {
    ...
    oauthMissing: info && Array.isArray(info.oauthMissing) ? info.oauthMissing : [],
    nickname: info && (info.nickname || info.displayName || info.userId) || fallback.nickname,
...
function renderUserBtn() {
  ...
  if (!btn) return;
  var externalProviders = accountProviderExternalRenderList();
  var topRight = document.getElementById('top-right');
  if (topRight) topRight.classList.toggle('account-pill-stack', externalProviders.length > 1);
  btn.classList.remove('multi-account', 'external-account-pills');
  if (externalProviders.length) {
    ...
    btn.title = '登录接入 / 平台胶囊展示';
    btn.innerHTML = externalProviders.map(function (provider) {
      return renderTopAccountPill(provider, { force: true });
    }).join('');
