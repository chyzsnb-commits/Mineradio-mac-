...
function updateMemoryControls() {
  ...
  // 系统级释放为 Windows 独有;主进程回传 systemPurgeAvailable===false(如 macOS)时整段隐藏,
  // 只保留内存状态 chip 与「压缩播放器」(mac 已接软压缩真实现)。
  var hideWindowsOnly = !!(memoryLastStatusPayload && memoryLastStatusPayload.systemPurgeAvailable === false);
  ['t-memoryAutoSystemTrim', 't-memorySystemAutoElevate', 'memory-mask-seg'].forEach(function (id) {
    var el = document.getElementById(id);
    if (el) el.style.display = hideWindowsOnly ? 'none' : '';
  });
  ['fx-memory-interval', 'fx-memory-threshold'].forEach(function (id) {
    ...
    if (row) row.style.display = hideWindowsOnly ? 'none' : '';
  });
  document.querySelectorAll('.memory-action-row button').forEach(function (btn, index) {
    if (index >= 1) btn.style.display = hideWindowsOnly ? 'none' : '';
  });
