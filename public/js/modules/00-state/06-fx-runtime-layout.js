var DEVELOPMENT_LOCKED_FX = {};   // 壁纸模式已解锁:macOS 走 desktop/wallpaper-mode.js(主窗口降桌面层,移植自主线 fork)
function isDevelopmentLockedFx(key) {
  return !!DEVELOPMENT_LOCKED_FX[key];
}
function normalizeDevelopmentLockedFxState() {
  if (!fx) return;
  // 只强制关掉「真正 dev-locked」的开关;壁纸模式已解锁,不能再无条件清零(否则 toggle 刚开就被这里打回 false)
  Object.keys(DEVELOPMENT_LOCKED_FX).forEach(function (k) { if (DEVELOPMENT_LOCKED_FX[k]) fx[k] = false; });
}
...
function clampPlaylistPanelFxSettings() {
  ...
  fx.playlistPanelOpenDuration = clampRange(fx.playlistPanelOpenDuration == null ? fxDefaults.playlistPanelOpenDuration : Number(fx.playlistPanelOpenDuration), 0.12, 0.90);
  fx.playlistPanelCloseDuration = clampRange(fx.playlistPanelCloseDuration == null ? fxDefaults.playlistPanelCloseDuration : Number(fx.playlistPanelCloseDuration), 0.08, 0.72);
}
