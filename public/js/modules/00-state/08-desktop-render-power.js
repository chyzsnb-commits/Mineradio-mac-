...
//  只调整 rank（实际生效的分析步长、流星门和体素额外量），不改用户分辨率、P10 密度或前台帧率。
...
function autoGovRollbackUp() {
  // 滞回：刚恢复品质就再次卡顿时，回退上一个 rank。
  autoGov.rank = Math.max(0, autoGov.rank - 1);
}
function autoGovTick() {
  syncGlassLiteClass();                                            // 覆盖用户显式 eco 档，即使主循环尚未就绪。
...
// 用户显式 eco 或自动治理已降到 eco 时，才减轻常驻玻璃位移合成。
function shouldEngageGlassLite() {
  ...
  return quality === 'auto' && performanceQualityRank() <= 0;
}
