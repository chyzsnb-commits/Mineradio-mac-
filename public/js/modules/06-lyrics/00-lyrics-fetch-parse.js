...
function lyricEndpointForSong(songOrId) {
  ...
  if (provider === 'kugou') {
    // 酷狗无专属歌词端点(id 是 Hash,打网易必空)→ 同走服务端跨源匹配(标题+歌手+时长)
    return '/api/ytmusic/lyric?title=' + encodeURIComponent(song.name || '') +
      '&artist=' + encodeURIComponent(song.artist || '') +
      '&sec=' + Math.round((Number(song.duration) || 0) / 1000);
  }
  if (provider === 'ytmusic') {
    // YT 无网易/QQ id,交服务端按清洗标题 + 歌手 + 时长跨源匹配取词
    return '/api/ytmusic/lyric?title=' + encodeURIComponent(song.name || '') +
      '&artist=' + encodeURIComponent(song.artist || '') +
...
// ---- 翻译借词的候选判定(与"换源播放"分开的更宽松匹配)----------------------------
// 为什么单独一套:sourceCandidateRejectReason(07-search.js)服务换源播放,要求标题归一后完全相等
// + 歌手重叠 + 拒翻唱——因为换源要找"可播的正版音源",放了翻唱就播错版本。但借"翻译"只需确认是
// 同一首歌:跨平台同名歌常有罗马音/括号副标题/feat 的细微差异,歌手写法也常不同,翻唱的中译通常也能用。
// 一套严格匹配套到翻译上 → 一失配就静默无翻译(QQ/酷狗/汽水 自身 trans 常空,基本要靠网易兜底)。
// 故这里以「标题高相似」为主锚点,歌手/时长只加分不硬否决,翻唱只降分不硬拒;阈值比换源宽,但标题
// 既不相等也不互含即直接否(防完全不相关的歌被借词)。改动只影响翻译兜底,不碰换源播放路径。
var LYRIC_TRANS_MATCH_ACCEPT_SCORE = 40;   // 经验阈值,可一行调:调低=更爱借词(误配风险↑),调高=更严
function translationCandidateMatchScore(song, candidate) {
  if (!song || !candidate) return -1;
  var st = simpleSearchNorm(song.name || song.title || '');
  var ct = simpleSearchNorm(candidate.name || candidate.title || '');
  if (!st || !ct) return -1;
  var score = 0;
  // 标题(主锚点):归一后相等 / 一方包含另一方(去括号副标题、罗马音后长度不同很常见)
  if (st === ct) {
    score += 60;
  } else if (st.length >= 2 && ct.length >= 2 && (st.indexOf(ct) >= 0 || ct.indexOf(st) >= 0)) {
    var ratio = Math.min(st.length, ct.length) / Math.max(st.length, ct.length);
    score += ratio >= 0.6 ? 48 : 34;   // 包含比例越高越可信;低比例(如短名嵌长名)需歌手/时长补分才过阈
  } else {
    return -1;   // 标题既不相等也不互含 → 不是同一首,直接否
  }
  // 歌手:重叠加分,不重叠不硬否决(翻译借词对歌手写法差异容忍度高,标题才是主锚点)
  if (typeof sourceSwitchArtistParts === 'function' && typeof sourceSwitchPartsOverlap === 'function') {
    var sp = sourceSwitchArtistParts(song), cp = sourceSwitchArtistParts(candidate);
    if (sp.length && cp.length && sourceSwitchPartsOverlap(sp, cp)) score += 20;
  }
  // 时长:有则 ±10s 加分,缺失不否决(单位统一为 ms;字段兼容 duration/dt)
  var sd = Number(song.duration) || 0;
  var cd = Number(candidate.duration || candidate.dt) || 0;
  if (sd > 0 && cd > 0 && Math.abs(sd - cd) <= 10000) score += 10;
  // 翻唱/衍生:轻惩罚不硬拒(翻唱的中译通常也能用)
  if (typeof searchLooksLikeDerivative === 'function') {
    var raw = String((candidate.name || '') + ' ' + (candidate.artist || '') + ' ' + (candidate.album || '')).toLowerCase();
    if (searchLooksLikeDerivative(raw)) score -= 14;
  }
  return score;
}
function translationCandidateAcceptable(song, candidate) {
  return translationCandidateMatchScore(song, candidate) >= LYRIC_TRANS_MATCH_ACCEPT_SCORE;
}
async function findNeteaseLyricFallbackCandidate(song) {
  var query = lyricNeteaseFallbackSearchQuery(song);
  if (!query) return null;
  var songName = (song && (song.name || song.title)) || '';
  ...
  if (!Array.isArray(list) || !list.length) {
    console.info('[LyricTransFallback]', '未命中', songName, '(搜索无结果)');
    return null;
  }
  // 用更宽松的翻译匹配打分挑最高分候选(替换掉原来复用换源严格匹配 + scoreSongSearchResult 的写法)
  var best = null, bestScore = -1;
  for (var i = 0; i < list.length; i++) {
    if (!list[i] || !list[i].id) continue;
    var sc = translationCandidateMatchScore(song, list[i]);
    if (sc >= LYRIC_TRANS_MATCH_ACCEPT_SCORE && sc > bestScore) { best = list[i]; bestScore = sc; }
  }
  if (best) console.info('[LyricTransFallback]', '命中', songName, '→ 网易《' + (best.name || best.title || '') + '》', 'score=' + bestScore);
  else console.info('[LyricTransFallback]', '未命中', songName, '(无候选达阈值 ' + LYRIC_TRANS_MATCH_ACCEPT_SCORE + ')');
  return best;
}
...
async function fetchLyric(songOrId, token, attempt) {
  ...
  try {
    ...
    cancelPendingTrackFallbackLyrics();
    var state = parseLyricResponseToOriginalState(song, r);
...
function renderLyrics(options) {
  ...
  if (typeof requestStageLyricWarmup === 'function') requestStageLyricWarmup(prewarmReason, fallbackTitleOnly ? 120 : 420);
