...
function updateLyricPaletteFromCover(coverCanvas) {
  ...
  try {
    ...
    var areaBuckets = {};
    var hist = Object.create(null);   // 粗量化直方图(RGB 每通道 4bit):选封面第一/第二主色 → dom1/dom2(体素柱体/冲击波用)
    for (var y = 0; y < h; y += 8) {
      for (var x = 0; x < w; x += 8) {
        var di = (y * w + x) * 4;
        ...
        var hk = (r >> 4) << 8 | (g >> 4) << 4 | (b >> 4);
        var hb = hist[hk];
        if (hb) { hb[0]++; hb[1] += r; hb[2] += g; hb[3] += b; } else hist[hk] = [1, r, g, b];
        sumChroma += chroma;
    ...
    // dom1/dom2 只追加字段不动已有输出:计数第一为主色;次色需与主色色相差 ≥30°(桶内均色比较),否则取第三
    var hkeys = Object.keys(hist).sort(function (ka, kb) { return hist[kb][0] - hist[ka][0]; });
    var histHex = function (k) {
      var hb = hist[k], n = hb[0];
      return '#' + ((1 << 24) | (Math.round(hb[1] / n) << 16) | (Math.round(hb[2] / n) << 8) | Math.round(hb[3] / n)).toString(16).slice(1);
    };
    var histHsl = function (k) { var hb = hist[k], n = hb[0]; return rgbToHsl(hb[1] / n, hb[2] / n, hb[3] / n); };
    var histDistinct = function (ka, kb) {
      var a = histHsl(ka), b = histHsl(kb);
      if (a.s < 0.14 || b.s < 0.14) {
        // 无彩色(白/灰/黑)没有色相, 色相门失效 → 用 RGB 距离判"够不同"(白底红标封面: 红应当选为次色)
        var ha = hist[ka], hb2 = hist[kb];
        var dr = ha[1] / ha[0] - hb2[1] / hb2[0], dg = ha[2] / ha[0] - hb2[2] / hb2[0], db = ha[3] / ha[0] - hb2[3] / hb2[0];
        return (dr * dr + dg * dg + db * db) > 3600;   // 距离 > 60/255 级
      }
    ...
    if (hkeys.length) {
      stageLyrics.coverPalette.dom1 = histHex(hkeys[0]);
      if (hkeys[1]) {
        stageLyrics.coverPalette.dom2 = histHex((histDistinct(hkeys[0], hkeys[1]) || !hkeys[2]) ? hkeys[1] : hkeys[2]);
      }
    }
...
// 解析结果按颜色字符串缓存 rgb:歌词调色板每帧用同一批颜色反复调用,
// 正则/setStyle/字符串解析是热路径浪费;仍返回全新可变 Color(调用方会改写亮度)。
var _cssColorRgbCache = Object.create(null);
function cssColorToThreeColor(css, fallback) {
  var c = new THREE.Color(fallback || '#d6f8ff');
  var value = String(css || fallback || '#d6f8ff').trim();
  var cached = _cssColorRgbCache[value];
  if (cached) { c.setRGB(cached[0], cached[1], cached[2]); return c; }
  try {
  ...
  _cssColorRgbCache[value] = [c.r, c.g, c.b];
  return c;
}
