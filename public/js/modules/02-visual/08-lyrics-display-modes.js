...
function lyricMotionProfile() {
  ...
  var profile = {
    ...
    glowLift: 1.0,
    floatAmp: 1.0,
    // ── 三档辨识度参数(默认全为「无操作」值,故 shine/glitch/quick/默认档逐位不变)──
    //   bob=竖向漂浮幅度倍率,bobRate=漂浮频率倍率
    bob: 1.0,
    bobRate: 1.0
  };
  if (style === 'smooth') {
    // 柔滑:稳如字幕——几乎不摆(bob/floatAmp 极小)、入出场拉长且更绵(黄油感)
    profile.enter = 1.06; profile.exit = 0.92; profile.slide = 0.16; profile.progressEase *= 0.72; profile.contextDrift = 0.022; profile.edgeBoost = 0.62; profile.sweep = 0.18; profile.shimmer = 0.05; profile.glowLift = 0.74; profile.floatAmp = 0.28; profile.bob = 0.12; profile.bobRate = 0.70;
  } else if (style === 'float') {
    // 漂浮:浮在水面——竖向漂移幅度拉大 ~2.6x、频率放慢(慢速上下摆)
    profile.enter = 0.86; profile.exit = 0.76; profile.slide = 0.54; profile.progressEase *= 0.66; profile.contextDrift = 0.135; profile.edgeBoost = 1.04; profile.sweep = 0.36; profile.shimmer = 0.14; profile.glowLift = 1.16; profile.floatAmp = 1.40; profile.bob = 2.6; profile.bobRate = 0.58;
  } else if (style === 'glass') {
    // 玻璃:干脆利落——入场快而脆、摆动极小(不做 scale 脉动/入场过冲:用户嫌"放大缩小放大",改回原来的稳定缩放,由「歌词缩放脉动」滑块按需开启)
    profile.enter = 0.42; profile.exit = 0.40; profile.slide = 0.30; profile.progressEase *= 0.96; profile.contextDrift = 0.050; profile.edgeBoost = 1.18; profile.sweep = 0.72; profile.shimmer = 0.22; profile.glowLift = 1.02; profile.floatAmp = 0.42; profile.bob = 0.22; profile.bobRate = 1.25;
  } else if (style === 'shine') {
