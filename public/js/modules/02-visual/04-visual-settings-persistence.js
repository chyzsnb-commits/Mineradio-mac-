...
function normalizePerformanceQuality(v) {
  var value = String(v || '');
  return /^(eco|balanced|high|ultra|auto)$/.test(value) ? value : fxDefaults.performanceQuality;
}
...
function readSavedLyricLayout() {
  ...
  try {
    ...
    var savedPlaylistPanelOpenDuration = clampRange(raw.playlistPanelOpenDuration == null ? fxDefaults.playlistPanelOpenDuration : Number(raw.playlistPanelOpenDuration), 0.12, 0.90);
    var savedPlaylistPanelCloseDuration = clampRange(raw.playlistPanelCloseDuration == null ? fxDefaults.playlistPanelCloseDuration : Number(raw.playlistPanelCloseDuration), 0.08, 0.72);
    ...
    var savedShelfMode = /^(side|stage|both)$/.test(String(raw.shelf || '')) ? raw.shelf : fxDefaults.shelf;
...
function currentFxAutosaveCriticalPatch() {
  ...
  return {
    ...
    shelf: /^(side|stage|both)$/.test(String(fx.shelf || '')) ? fx.shelf : fxDefaults.shelf,
...
function saveLyricLayout(opts) {
  ...
  try {
    ...
    var autosavePayload = {
      ...
      playlistPanelOpenDuration: clampRange(fx.playlistPanelOpenDuration == null ? fxDefaults.playlistPanelOpenDuration : Number(fx.playlistPanelOpenDuration), 0.12, 0.90),
      playlistPanelCloseDuration: clampRange(fx.playlistPanelCloseDuration == null ? fxDefaults.playlistPanelCloseDuration : Number(fx.playlistPanelCloseDuration), 0.08, 0.72),
