...
function rainGlassSpeedValue() {
  var v = (typeof fx !== 'undefined' && fx && isFinite(fx.rainGlassSpeed)) ? Number(fx.rainGlassSpeed) : 5.00;
  return Math.max(0.2, Math.min(16, v));
}
...
function ensureRainMood() {
  ...
  // 复用一条轻量折线，随机雷闪时短暂显现，不为每次雷击创建新几何。
  var lightningGeo = new THREE.BufferGeometry();
  lightningGeo.setAttribute('position', new THREE.BufferAttribute(new Float32Array(48), 3));
  lightningGeo.setDrawRange(0, 0);
  var lightningMat = new THREE.LineBasicMaterial({
    color: 0xeaf2ff,
    transparent: true,
    opacity: 0,
    depthWrite: false,
  ...
  var lightning = new THREE.LineSegments(lightningGeo, lightningMat);
...
function rainMoodDrawLightning(rm, strength) {
  if (!rm || !rm.lightning || !rm.lightning.geometry || !rm.lightning.material) return;
  var attr = rm.lightning.geometry.attributes.position;
  if (!attr) return;
  var pos = attr.array;
  var cursor = 0;
  ...
  // 主叉六段，局部再分出一条支叉；全程复用同一 position buffer。
  for (var i = 0; i < 6; i++) {
    x += (Math.random() - 0.5) * 2.5;
    y -= 2.2 + Math.random() * 1.2;
    pos[cursor++] = lastX; pos[cursor++] = lastY; pos[cursor++] = z;
    pos[cursor++] = x; pos[cursor++] = y; pos[cursor++] = z;
    if (i === 2) {
      pos[cursor++] = x; pos[cursor++] = y; pos[cursor++] = z;
      pos[cursor++] = x + (Math.random() < 0.5 ? -1 : 1) * (1.5 + Math.random() * 1.5);
      pos[cursor++] = y - (1.2 + Math.random() * 1.1); pos[cursor++] = z;
    }
    lastX = x;
    lastY = y;
  }
  attr.needsUpdate = true;
  rm.lightning.geometry.setDrawRange(0, cursor / 3);
  rm.lightningAmt = Math.max(rm.lightningAmt || 0, strength);
  rm.lightning.material.opacity = 0.18 + strength * 0.72;
  rm.lightning.visible = true;
}

function rainMoodQueueRandomThunder(rm) {
  if (!rm) return;
  var flashCount = Math.random() < 0.38 ? 2 + Math.floor(Math.random() * 2) : 1;
  var offset = 0;
  rm.thunderFlashes.length = 0;
  for (var i = 0; i < flashCount; i++) {
    rm.thunderFlashes.push({
      at: rainMoodClock + offset,
      strength: 0.50 + Math.random() * 0.32
    });
    offset += 0.065 + Math.random() * 0.12;
  }
}

function rainMoodConsumeThunderFlash(rm) {
  if (!rm || !rm.thunderFlashes || !rm.thunderFlashes.length) return 0;
  var strength = 0;
  while (rm.thunderFlashes.length && rainMoodClock >= rm.thunderFlashes[0].at) {
    var flash = rm.thunderFlashes.shift();
    strength = Math.max(strength, flash.strength);
  }
  if (strength > 0) rainMoodDrawLightning(rm, strength);
  return strength;
}
...
function updateRainMood(dt) {
  ...
  flashTarget = Math.max(flashTarget, rainMoodConsumeThunderFlash(rm));
  ...
  rm.lightningAmt = rainMoodEase(rm.lightningAmt, 0, 0.55, 0.38, step);
  if (rm.lightning && rm.lightning.material) {
    rm.lightning.material.opacity = (0.18 + rm.lightningAmt * 0.72) * Math.min(1, rm.lightningAmt * 2.2);
    rm.lightning.visible = rm.lightningAmt > 0.025;
  }
