...
function normalizeCustomBackgroundMedia(value) {
  ...
  if (type === 'image') {
    ...
    var imageId = String(value.id || '').trim();
    if (!imageSrc && !imageId) return null;   // 允许「按 id 存在 IndexedDB 的图片」(网格导入的图片),不再强制内联 src
    return {
      type: 'image',
      id: imageId,
      src: imageSrc,
...
var customBgImageObjectUrl = '';   // 网格导入的图片(按 id 存 IndexedDB)应用时的 objectURL 生命周期
...
function applyColorLabValue(hex, silent) {
  ...
  else if (id === 'vox-color-pick' || id === 'vox-ripple-pick') {
    // 体素双色: 分发到各自 setter 并回写取色块(swatch 颜色与实际生效值一致)
    if (id === 'vox-color-pick' && typeof voxSetColor === 'function') voxSetColor(hex);
    else if (typeof voxSetRippleColor === 'function') voxSetRippleColor(hex);
    var voxPick = document.getElementById(id);
    if (voxPick) voxPick.value = hex;
  }
