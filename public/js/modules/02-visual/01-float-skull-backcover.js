...
var skullLyricMouthLocal = new THREE.Vector3(0.025, -0.45, 0.62);   // 用户要歌词上移一点(原 -0.72 落在下巴,抬到嘴/鼻区)
...
function setSkullCameraTargetVectors(pos, look, portrait, shelfComposition, zoom) {
  zoom = Number(zoom) || 0;
  // 用户定的正面居中机位(#45):整颗骷髅正对镜头、略俯视,拉远到整颗完整可见。
  // 歌单架开合都用同一机位(不再把骷髅左移让位)——歌架改到右侧独立呈现。
  pos.set(0.00, 0.90, (portrait ? 7.60 : 6.70) + zoom);
  look.set(0.00, 0.15, 0.00);
}
...
function updateSkullParticleLayer(dt) {
  ...
  if (typeof skullGestureFlash === 'number' && skullGestureFlash > 0.004) flashTarget = Math.max(flashTarget, Math.min(1, skullGestureFlash));   // v9 手势:握拳触发闪光
  ...
  // 骷髅固定正面居中(#45):歌单架开合不再放大/左移骷髅,shelf* 目标一律取基准值
  var shelfScale = SKULL_MODEL_SCALE;
  ...
  var shelfX = SKULL_MODEL_BASE_POSITION.x;
  var shelfY = SKULL_MODEL_BASE_POSITION.y;
