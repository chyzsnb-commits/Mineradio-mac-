...
var uniforms = {
  ...
  uHandActive: { value: 0 },
  uHand2XY: { value: new THREE.Vector2(-999, -999) },   // v9 第二只手
  uHand2Active: { value: 0 },
...
uniform vec2 uMouseXY, uHandXY, uHand2XY;
uniform float uHandActive, uHand2Active, uGestureGrip;
...
if (coverMask > 0.02) {
  ...
  //  (Preset 6 SKULL 也落在此分支——主粒子由 JS 侧隐藏，仅占位)
  // ====================================================
  else if (uPreset < 6.5) {
...
if (transition > 0.001) {
  ...
  // ====================================================
  //  Preset 7: BLACK HOLE — 吸积盘漩涡
  // ====================================================
  else if (uPreset < 7.5) {
float rad = mix(0.35, 3.4, sqrt(aUv.y));          // 内密外疏
float swirl = aUv.x * 2.0 * PI + t * (0.5 + (1.0 - aUv.y) * 1.8) + log(rad + 0.2) * 2.4;
float suck = uBass * 0.6 * K + uBeat * 0.5;       // 低频把粒子往里吸
float r = max(0.18, rad - suck * (1.0 - aUv.y) * 0.5);
vec3 disk = vec3(cos(swirl) * r, (hash11(aRand * 51.0) - 0.5) * r * 0.10, sin(swirl) * r);
float tilt = 0.55;                                // 盘面倾斜
float ct = cos(tilt), st = sin(tilt);
disk.yz = mat2(ct, -st, st, ct) * disk.yz;
pos = disk;
float horizon = smoothstep(0.18, 0.7, r);         // 事件视界：越靠中心越透
vAlpha = horizon;
vColor = mix(vColor, vec3(1.0, 0.85, 0.6), (1.0 - horizon) * 0.4);  // 内缘暖辉
maxRippleAmp = max(maxRippleAmp, (1.0 - horizon) * (0.3 + uTreble * 0.4) + uBeat * 0.3);
  }

  // ====================================================
  //  Preset 8: AURORA — 极光丝带 (几片上下翻涌的丝带)
  // ====================================================
  else if (uPreset < 8.5) {
float NC = 6.0;                                   // 6 道光帘
float cid = floor(aUv.x * NC);                    // 第几道
float fr = fract(aUv.x * NC);                     // 帘内横向 0..1
float cseed = hash11(cid * 23.0 + 5.0);
float h = aUv.y;                                  // 0底..1顶
float yPos = (h - 0.5) * 12.0;                    // 竖直铺满
float colX = (cid / (NC - 1.0) - 0.5) * 18.0;     // 横向铺满 ±9
// 随高度左右翻涌（极光飘动），低/中频加强
float undulate = sin(h * 2.2 + t * (0.5 + cseed) + cseed * 6.28) * (1.2 + uBass * 1.6 * K)
               + sin(h * 4.5 - t * 0.8 + cseed * 3.0) * (0.5 + uMid * 0.8 * K);
float width = (fr - 0.5) * 2.2;                   // 帘宽
pos.x = colX + undulate + width;
pos.y = yPos;
pos.z = sin(cid + t * 0.4) * 2.0 + width * 0.3;
// 颜色：底青绿 → 顶紫粉
vec3 lo = vec3(0.25, 1.0, 0.55);
vec3 hi = vec3(0.65, 0.40, 1.0);
vColor = mix(vColor, mix(lo, hi, smoothstep(0.0, 1.0, h)), 0.62);
// 帘中心亮、两侧渐隐；底实顶飘逸
float edge = clamp(1.0 - abs(fr - 0.5) * 1.7, 0.0, 1.0);
float vFade = smoothstep(0.0, 0.18, h) * (1.0 - smoothstep(0.78, 1.0, h) * 0.6);
vAlpha = edge * vFade * (0.45 + uMid * 0.5 + uTreble * 0.3);
maxRippleAmp = max(maxRippleAmp, edge * (0.2 + uBeat * 0.4 + uTreble * 0.25));
  }
...
if (hd < rad) {
  ...
  if (uHand2Active > 0.01) {   // v9 第二只手同款推开场
float h2dx = pos.x - uHand2XY.x;
float h2dy = pos.y - uHand2XY.y;
float h2d = sqrt(h2dx*h2dx + h2dy*h2dy);
float rad2 = 1.55;
if (h2d < rad2) {
  float push2 = (rad2 - h2d) / rad2;
  push2 = push2 * push2 * uHand2Active;
  pos.z += push2 * 1.10;
  vec2 outDir2 = vec2(h2dx, h2dy) / max(0.001, h2d);
  pos.xy += outDir2 * push2 * 0.28;
}
  }
