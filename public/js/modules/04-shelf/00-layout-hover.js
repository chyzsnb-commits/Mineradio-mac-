...
function shelfVoxelWorldScale() {
  ...
  // 按当前相机距离放大歌架,保留有限上下界,避免用户切换体素密度时尺寸失控。
  var radius = 102.9;
  if (typeof _voxCam !== 'undefined' && _voxCam && isFinite(Number(_voxCam.radius))) radius = Number(_voxCam.radius);
  else if (typeof camera !== 'undefined' && camera && camera.position && typeof camera.position.length === 'function') radius = camera.position.length();
  return clampRange(radius / 7.0, 8.0, 18.0);
}
function shelfLayoutProfile() {
  ...
  var voxelShelf = typeof voxelCityActive === 'function' && voxelCityActive();
  var voxelShelfScale = shelfVoxelWorldScale();
  ...
  return {
    portrait: portrait,
    narrow: narrow,
    sideX: ((skullShelf ? (portrait ? 0.22 : (narrow ? 0.46 : 0.76)) : (portrait ? 1.56 : (narrow ? 2.48 : 3.18))) + shelfCtl.x) * shelfOffsetScale,
    sideY: ((skullShelf ? (portrait ? -0.22 : (narrow ? -0.30 : -0.34)) : 0) + shelfCtl.y) * shelfOffsetScale,
    sideXStep: (skullShelf ? (portrait ? 0.018 : 0.034) : (portrait ? 0.018 : 0.040)) * sideScaleFactor,
    sideYStep: (skullShelf ? (portrait ? 0.46 : 0.62) : (portrait ? 0.52 : 0.68)) * sideScaleFactor,
    sideZ: ((skullShelf ? (portrait ? 0.86 : 0.92) : (portrait ? 0.78 : 0.86)) + shelfCtl.z) * shelfOffsetScale,
    sideZStep: (skullShelf ? (portrait ? 0.108 : 0.158) : (portrait ? 0.118 : 0.170)) * sideScaleFactor,
