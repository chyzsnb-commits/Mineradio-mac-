function makeShelfManager() {
  ...
  // ====================================================
  function placeCard(card, i, totalCards, modeIs, layout, shelfLook, summon) {
...
void main(){ vec4 t = texture2D(uDotTex, gl_PointCoord); if (t.a < 0.02) discard; gl_FragColor = vec4(vC, t.a * vA); }`,
  ...
  return {
    ...
    update: function (dt) {
      ...
      // fork 灵敏度: 自动隐藏整体可见度缓动(用户觉得太快, 调低到 0.14 / 0.11 让淡入淡出更缓);
      // 乘 dt 归一化(60fps 基准),避免 120Hz 高刷下淡出快一倍。
      if (shelfHardHidden) targetVis = 0;   // 播放栏「关闭3D歌架」按钮:硬隐藏,优先级最高
      ...
      // 右侧 3D 歌单架对所有普通预设保持可用,包括音域回响;只有壁纸模式才需要隐藏
      // (它是 3D 场景物体,mw-wallpaper 的 CSS 只隐藏 DOM,管不到它)。
      ...
      for (var i = 0; i < cards.length; i++) {
        placeCard(cards[i], i, cards.length, mode, frameLayout, frameShelfLook, frameSummon);
      }
      // 内容更新 (节流)
