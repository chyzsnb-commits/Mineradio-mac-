...
function makeContentListManager() {
  ...
  function drawPanel() {
    ...
    var panelBgAlpha = shelfGlassAlpha();
    bg.addColorStop(0, shelfGlassRgba(Math.min(0.72, panelBgAlpha + 0.06), 1.0));
    bg.addColorStop(0.42, shelfGlassRgba(panelBgAlpha, 0.78));
    bg.addColorStop(1, shelfGlassRgba(Math.max(0.18, panelBgAlpha - 0.08), 0.42));
  ...
  async function loadMoreContentRows(reason) {
    ...
    try {
      ...
      if (contentTotalCount && contentNextOffset >= contentTotalCount) contentHasMore = false;
  ...
  function drawRow(row, song, isCenter) {
    ...
    var rowBgAlpha = shelfGlassAlpha();
    var centerRowBgAlpha = isCenter ? Math.max(rowBgAlpha, 0.62) : rowBgAlpha;
    if (isCenter) {
      rowGrad.addColorStop(0, shelfGlassRgba(Math.min(0.70, centerRowBgAlpha + 0.040), 1.0));
      rowGrad.addColorStop(0.48, shelfGlassRgba(Math.min(0.68, centerRowBgAlpha + 0.030), 0.78));
      rowGrad.addColorStop(1, shelfGlassRgba(Math.min(0.66, centerRowBgAlpha + 0.015), 0.46));
    } else {
      rowGrad.addColorStop(0, shelfGlassRgba(Math.max(0.22, rowBgAlpha - 0.02), 0.74));
      rowGrad.addColorStop(1, shelfGlassRgba(Math.max(0.18, rowBgAlpha - 0.06), 0.36));
    }
    if (isCenter) {
  ...
  return {
    ...
    open: async function (playlistId, title, fromCard) {
      ...
      contentSource = podcastCollectionKey ? null : {
        provider: qqPlaylistId ? 'qq' : 'netease',
        id: qqPlaylistId || playlistId
      };
      // 拉取歌单/播客集合
      var r = null;
      try {
    ...
    playRow: function (row) {
      ...
      if (contentHasMore || (contentTotalCount && allTracks.length < contentTotalCount)) {
        showToast('超大歌单已先播放当前已加载部分，继续滚动会加载更多');
      }
      playQueue = allSongs;
