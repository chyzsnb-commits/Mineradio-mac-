async function analyzeAudioBeats(audioUrl, durationSec, token, options) {
  ...
  // 用临时 AudioContext 解码 (我们不能复用 audioCtx 因为它可能 closed); 提前声明是因为 partial 流水线可能先跑到 renderBand
  ...
  var beatFullMapDone = false; // 全曲图已产出后, 迟到的 partial 一律丢弃, 防止半图倒灌覆盖

  // 前30秒快速首段分析(与全曲主流程并行): Range 拉前 ~1.5MB 截断解码(MP3/FLAC 通常可解截断流),
  // 跳过 music-tempo; 任何一步失败都静默放弃, 主流程照旧
  function startPartialBeatAnalysis() {
    (async function () {
      if (!TmpCtx || !DecodeCtx) return;
      var pResp = await fetch(audioUrl, { headers: { Range: 'bytes=0-1572863' } });
      // 非 206 = 服务端忽略 Range, 继续读 body 会把全曲并行下两遍, 放弃(必须 cancel, 否则浏览器仍在后台拉整曲)
      if (pResp.status !== 206) {
        try { if (pResp.body && pResp.body.cancel) pResp.body.cancel(); } catch (e) { }
        return;
      }
      var pAb = await pResp.arrayBuffer();
      if (token !== beatMapToken || beatFullMapDone) return;
      var pdc = new DecodeCtx();
      var pBuffer = await new Promise(function (resolve, reject) {
        pdc.decodeAudioData(pAb, resolve, reject);
      }).catch(function () { return null; });
      if (pBuffer && pBuffer.duration > 30.5) {
        // 只留前 30s, 截断解码的尾部残帧一并丢掉
        var sliceLen = Math.min(pBuffer.length, Math.floor(pBuffer.sampleRate * 30));
        var sliced = pdc.createBuffer(pBuffer.numberOfChannels, sliceLen, pBuffer.sampleRate);
        var chScratch = new Float32Array(sliceLen);
        for (var pch = 0; pch < pBuffer.numberOfChannels; pch++) {
          pBuffer.copyFromChannel(chScratch, pch, 0);
          sliced.copyToChannel(chScratch, pch, 0);
        }
        pBuffer = sliced;
      }
      pdc.close && pdc.close();
      // 首段太短时峰选/percentile 统计不可靠, 放弃
      if (!pBuffer || pBuffer.duration < 8) return;
      if (token !== beatMapToken || beatFullMapDone) return;
      var pMap = await analyzeBeatBuffer(pBuffer, { skipMusicTempo: true });
      if (!pMap || token !== beatMapToken || beatFullMapDone) return;
      pMap.partial = true;
      pMap.partialUntilSec = pBuffer.duration;
      options.onPartial(pMap);
    })().catch(function (e) {
      console.warn('partial beat analysis skipped:', e && e.message ? e.message : e);
    });
  }

  // buffer → beatMap 主干(频段渲染/帧能量/峰选/music-tempo 融合), partial 与全曲共用同一条管线;
  // scratch 状态按调用隔离, 两条流水线可并行
  async function analyzeBeatBuffer(buffer, runOpts) {
    runOpts = runOpts || {};
    var percentileScratch = null;
    var musicTempoBeats = [];
    var musicTempoGridStep = 0;
    var musicTempoTask = (options.skipMusicTempo || runOpts.skipMusicTempo) ? Promise.resolve(null) : analyzeMusicTempoInWorker(buffer, token);
    ...
    function percentile(arr, p) {
      // 复用 Float32Array scratch,避免每次 percentile 都新建数组(减少 GC)
      var len = Math.min(nFrames || arr.length || 0, arr.length || 0);
      if (!len) return 0.001;
      if (!percentileScratch || percentileScratch.length < len) percentileScratch = new Float32Array(len);
      for (var pi = 0; pi < len; pi++) percentileScratch[pi] = arr[pi];
      var copy = percentileScratch.subarray(0, len);
      copy.sort();
      return copy[Math.floor(len * p)] || 0.001;
    }
    ...
    // —— 自相关 BPM 八度仲裁(第一步: 量 bpmAC) ——
    // 对低频 onset 包络(掺少量鼓身)做去均值自相关, lag 26..172 帧(10ms帧 ⇒ ~230..35BPM);
    // score(lag)=AC(lag)+0.4*AC(2*lag)(倍频谐波在界内才加分, 偏向真八度);
    // 主峰/次峰>1.25 才视为可信, 只用于下方纠正 music-tempo 的 2x/0.5x 八度错判
    var bpmAC = 0;
    var bpmACRatio = 0;
    var AC_MIN_LAG = 26, AC_MAX_LAG = 172;
    if (nFrames > AC_MAX_LAG * 2) {
      var acEnv = new Float32Array(nFrames);
      var acMean = 0;
      for (var ae = 0; ae < nFrames; ae++) {
        acEnv[ae] = onset[ae] + bodyOnset[ae] * 0.35;
        acMean += acEnv[ae];
      }
      acMean /= nFrames;
      var acNorm = 0;
      for (var av = 0; av < nFrames; av++) {
        var acDev = acEnv[av] - acMean;
        acEnv[av] = acDev;
        acNorm += acDev * acDev;
      }
      acNorm = Math.max(1e-9, acNorm / nFrames);
      var acRaw = new Float32Array(AC_MAX_LAG + 1);
      for (var acLag = AC_MIN_LAG; acLag <= AC_MAX_LAG; acLag++) {
        var acSum = 0;
        for (var af = 0; af + acLag < nFrames; af++) acSum += acEnv[af] * acEnv[af + acLag];
        acRaw[acLag] = Math.max(0, acSum / ((nFrames - acLag) * acNorm));
      }
      var acBestLag = 0, acBestScore = 0;
      for (var as1 = AC_MIN_LAG; as1 <= AC_MAX_LAG; as1++) {
        var acScore = acRaw[as1] + (as1 * 2 <= AC_MAX_LAG ? acRaw[as1 * 2] * 0.4 : 0);
        if (acScore > acBestScore) { acBestScore = acScore; acBestLag = as1; }
      }
      if (acBestLag > 0 && acBestScore > 0) {
        // 次峰须避开主峰邻域(±15%), 否则主峰肩部会把置信比压到 ~1
        var acSecond = 0;
        var acGuard = Math.max(3, Math.round(acBestLag * 0.15));
        for (var as2 = AC_MIN_LAG; as2 <= AC_MAX_LAG; as2++) {
          if (Math.abs(as2 - acBestLag) <= acGuard) continue;
          var acScore2 = acRaw[as2] + (as2 * 2 <= AC_MAX_LAG ? acRaw[as2 * 2] * 0.4 : 0);
          if (acScore2 > acSecond) acSecond = acScore2;
        }
        bpmAC = 6000 / acBestLag;
        bpmACRatio = acSecond > 0 ? acBestScore / acSecond : 9;
      }
    }
    ...
    if (musicTempoBeats.length >= 4) {
      // —— 自相关八度仲裁(第二步: 纠偏) ——
      // music-tempo 步长恰为 AC 步长的 ~2x(慢一个八度)或 ~0.5x(快一个八度)且 AC 主峰足够突出时,
      // 对拍点中点加密/隔一抽稀, 再交给下方既有 music-tempo 融合重建; 比值含糊或置信不足则完全不动
      if (bpmAC > 0 && bpmACRatio > 1.25 && musicTempoGridStep > 0) {
        var acStep = 60 / bpmAC;
        var octRatio = musicTempoGridStep / acStep;
        if (Math.abs(octRatio - 2) < 0.12) {
          var acDense = [];
          for (var od = 0; od < musicTempoBeats.length; od++) {
            acDense.push(musicTempoBeats[od]);
            if (od < musicTempoBeats.length - 1) {
              var odGap = musicTempoBeats[od + 1] - musicTempoBeats[od];
              // 只在正常网格间隔内插中点, 乐句间大空洞不硬填
              if (odGap > musicTempoGridStep * 0.7 && odGap < musicTempoGridStep * 1.4) acDense.push(musicTempoBeats[od] + odGap * 0.5);
            }
          }
          musicTempoBeats = acDense;
          musicTempoGridStep *= 0.5;
          console.log('bpmAC octave fix: densify to', (60 / musicTempoGridStep).toFixed(1), 'bpm (AC ' + bpmAC.toFixed(1) + ', ratio ' + bpmACRatio.toFixed(2) + ')');
        } else if (Math.abs(octRatio - 0.5) < 0.03) {
          // 抽稀留哪个相位由低频 onset 决定, 避免全落在反拍上
          var acPh0 = 0, acPh1 = 0;
          for (var op = 0; op < musicTempoBeats.length; op++) {
            var opFrame = Math.max(0, Math.min(nFrames - 1, Math.round(musicTempoBeats[op] / 0.010)));
            if (op % 2 === 0) acPh0 += onset[opFrame]; else acPh1 += onset[opFrame];
          }
          var acKeep = acPh1 > acPh0 ? 1 : 0;
          musicTempoBeats = musicTempoBeats.filter(function (t2, i2) { return i2 % 2 === acKeep; });
          musicTempoGridStep *= 2;
          console.log('bpmAC octave fix: thin to', (60 / musicTempoGridStep).toFixed(1), 'bpm (AC ' + bpmAC.toFixed(1) + ', ratio ' + bpmACRatio.toFixed(2) + ')');
        }
      }
    ...
    await yieldToPaint();
    if (token !== beatMapToken) { hideBeatChip(); return null; }
    // v10.2 全曲响度参考(10ms 帧 RMS 的 p95, 每 40ms 采一窗): 安静门"相对全曲"的参照——
    // 没有它, 前奏/慢歌在副歌出现之前无从判静, "海浪感"会在开头漏网
    var lrData = buffer.getChannelData(0);
    var lrWin = Math.max(1, Math.floor(buffer.sampleRate * 0.01));
    var lrFrames = [];
    for (var lrI = 0; lrI + lrWin <= lrData.length; lrI += lrWin * 4) {
      var lrS = 0;
      for (var lrJ = lrI; lrJ < lrI + lrWin; lrJ++) { var lv = lrData[lrJ]; lrS += lv * lv; }
      lrFrames.push(Math.sqrt(lrS / lrWin));
    }
    lrFrames.sort(function (a, b) { return a - b; });
    var loudRef = lrFrames.length ? lrFrames[Math.floor(lrFrames.length * 0.95)] : 0;
    if (options.prefetch) hideBeatChip();
    else showBeatChip('节奏缓冲中…');
    return { kicks: kicks, beats: beats, pulseBeats: pulseBeats, cameraBeats: cameraBeats, gridStep: gridStep, bpmAC: bpmAC ? Math.round(bpmAC * 10) / 10 : 0, loudRef: Math.round(loudRef * 1000) / 1000, tempoSource: musicTempoBeats.length >= 4 ? 'music-tempo' : 'local', analysisProfile: analysisProfile.id || 'default', duration: buffer.duration, visualBeatCount: visualBeatCount, analyzedAt: Date.now() };
  }
  ...
  try {
    ...
    showBeatChip('正在分析节奏…');
    if (typeof options.onPartial === 'function' && !options.prefetch) startPartialBeatAnalysis();
    ...
    if (token !== beatMapToken) { hideBeatChip(); return null; }
    var map = await analyzeBeatBuffer(buffer, null);
    if (map) beatFullMapDone = true;
    return map;
  } catch (e) {
