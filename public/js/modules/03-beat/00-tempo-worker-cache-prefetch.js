...
// Hi-Res/无损当前曲目:节拍分析改用 320k 流(同一首歌节拍一致), 避免全量解码 24bit FLAC 与播放解码抢资源出爆音;
// 解析失败回退原流, 行为不劣化
async function resolveBeatAnalysisLightUrl(song, fallbackUrl) {
  try {
    var pk = typeof songProviderKey === 'function' ? songProviderKey(song) : '';
    if (pk !== 'qq' && pk !== 'netease') return fallbackUrl;
    var isQQ = pk === 'qq';
    if (!isQQ && !(song && song.id)) return fallbackUrl;
    var q = normalizePlaybackQualityForProvider('exhigh', isQQ ? 'qq' : 'netease');
    var data = isQQ
      ? await apiJson('/api/qq/song/url?mid=' + encodeURIComponent(song.mid || song.songmid || song.id || '') + '&mediaMid=' + encodeURIComponent(song.mediaMid || song.media_mid || '') + '&quality=' + encodeURIComponent(q))
      : await apiJson('/api/song/url?id=' + encodeURIComponent(song.id) + '&quality=' + encodeURIComponent(q));
    if (data && data.url && !data.trial) return '/api/audio?url=' + encodeURIComponent(data.url);
  } catch (e) { }
  return fallbackUrl;
}
  ...
  beatAnalysisTimer = setTimeout(function waitForQuietStart() {
    ...
    var startAnalysis = async function () {
      ...
      analyzeAudioBeats(analysisUrl, null, token, {
        ...
        background: true,
        song: song || null,
        onPartial: function (partialMap) {
          // 前30秒半图提前装上; 全曲图已进缓存说明晚到了, 丢弃
          if (token !== beatMapToken || !partialMap || beatMapCache[songId]) return;
          smoothBeatMapHandoff(songId, partialMap, token, song || null);
        }
      }).then(function (map) {
...
async function writeBeatDiskCache(key, map, song, mode) {
  ...
  if (map.partial) return false; // 前30秒 partial 半图绝不落盘, 防止污染全曲缓存
  var st = await ensureBeatDiskCacheStatus();
...
async function fetchBeatPrefetchAudioUrl(song) {
  ...
  // 节拍分析固定 320k:节拍内容与音质无关, 无损/Hi-Res 全量解码太重, 会与正在播放的解码抢资源出爆音
  var requestedQuality = normalizePlaybackQualityForProvider('exhigh', isQQ ? 'qq' : 'netease');
