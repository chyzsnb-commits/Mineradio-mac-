...
const QISHUI_RELATED_MEDIA_PATH = '/api/douyin/v1/qishui/music/related/media/';
const QISHUI_FEED_SONG_TAB_PATH = '/api/douyin/v1/qishui/music/feed/song/tab/';
const QISHUI_SCOPE = 'luna.openapi.platform.play_core';
const DEFAULT_QISHUI_TOKEN_FILE = path.join(__dirname, '.qishui-token');
const QISHUI_UA = 'Mineradio/1.1.2 (Qishui official OpenAPI bridge)';
...
const QISHUI_PUBLIC_HEADERS = {
  ...
  'User-Agent': 'Mineradio/1.1.2 (Qishui public catalog bridge)',
};
...
function getQishuiStatus(cookieText) {
  ...
  return {
    ...
    message: webSession
      ? '汽水网页登录态已保存，可同步汽水歌单与我的喜欢；播放仍会按匹配源自动换源。'
      : tokenConfigured
      ? '汽水开放平台 token 已配置，可使用官方推荐/相关歌曲能力。'
      : (oauthConfig.configured
        ? '可通过抖音开放平台扫码授权获取 access-token；未授权时会先使用汽水公开搜索作为匹配源。'
        : (QISHUI_PUBLIC_ENABLED
          ? '汽水可先用公开搜索/歌词匹配；预置抖音开放平台凭证后可扫码授权增强推荐能力。'
          : '汽水公开搜索已关闭；预置抖音开放平台凭证后可扫码授权增强推荐能力。')),
  };
}
...
function qishuiStreamFromObject(value, inherited) {
  ...
  return {
    ...
    size: qishuiObjectNumber(value, ['size', 'Size', 'file_size', 'FileSize', 'data_size', 'DataSize']) ||
    ...
    bitrate: qishuiObjectNumber(value, ['bitrate', 'Bitrate', 'real_bitrate', 'RealBitrate', 'br', 'BR', 'bit_rate', 'BitRate']) ||
    ...
    duration: qishuiNormalizeDurationSeconds(qishuiObjectNumber(value, ['duration', 'Duration']) || inherited.duration || 0),
  };
}
...
function cacheQishuiLyric(id, lyric, tlyric, source) {
  id = normalizeText(id);
  lyric = normalizeLyricBody(lyric);
  tlyric = normalizeLyricBody(tlyric);
      ...
      try {
        ...
        extractQishuiPlaylistCards(json).forEach(pl => {
          ...
          if (/created/i.test(label) || primaryLike) {
            ...
            pl.owned = true;
            pl.subscribed = false;
          } else if (/collection|collect/i.test(label)) {
            pl.shelfPane = 'fav';
            pl.owned = false;
            pl.subscribed = true;
          }
...
async function fetchQishuiWebPlaylistTracks(playlistId, cookieText, opts) {
  ...
  return qishuiWebPlaylistCache.wrap(cacheKey, 90 * 1000, async () => {
    const targetCount = Math.min(240, offset + limit);
    const allRawItems = [];
    let cursor = '';
    let lastJson = null;
    let upstreamHasMore = true;
    for (let page = 0; page < 8 && allRawItems.length < targetCount && upstreamHasMore; page++) {
      const json = await qishuiWebRequestJson('/luna/pc/playlist/detail', qishuiPcAppParams({
        playlist_id: id,
        cursor,
        count: Math.min(100, Math.max(1, targetCount - allRawItems.length)),
      }), cookie, {
        bases: [QISHUI_WEB_PC_API_BASE],
        noDefaultParams: true,
        sessionOnly: true,
      ...
      cursor = normalizeText(pageData.next_cursor || pageData.nextCursor || json && json.next_cursor || '');
      upstreamHasMore = !!(pageData.has_more || pageData.hasMore || json && json.has_more) && !!cursor;
      if (!pageRawItems.length && !cursor) break;
    }
...
async function handleQishuiPublicSearch(keywords, limit) {
  const offset = 0;
  ...
  const songs = list.map((item, index) => mapQishuiPublicItem(item, index, keywords)).filter(Boolean).slice(0, limit);
  return {
    provider: 'qishui',
...
async function handleQishuiSearch(keywords, limit, opts) {
  ...
  return qishuiSearchCache.wrap(cacheKey, 2 * 60 * 1000, async () => {
    if (!status.tokenConfigured) return handleQishuiPublicSearch(keywords, limit);
    const payload = {
    ...
    try {
      ...
      const fallback = await handleQishuiPublicSearch(keywords, limit);
    ...
    try {
      ...
      if (detail && (detail.lyric || detail.tlyric)) {
        return {
          ...
          yrc: '',
          ytlrc: '',
          source: 'qishui-public-detail',
        };
      }
    } catch (err) {
      return { provider: 'qishui', lyric: '', tlyric: '', yrc: '', ytlrc: '', source: 'qishui-public-error', error: err.message };
    }
  }
...
module.exports = {
  ...
  // Q3 实验(pc-qr-web)需要在 Electron 主进程里同源复用这些 passport 请求构造件,纯附加导出,不改行为。
  QISHUI_PC_FIXED,
  qishuiPcUrl,
