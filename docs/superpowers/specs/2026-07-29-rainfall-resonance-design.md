...
新增独立的 11 号视觉预设「云瀑共振」，把歌曲频谱转成具有音乐喷泉感的分区雨幕：低频控制雨势与雨线厚度，中频控制横向列的旋律起伏，高频增加细雨亮点，拍点触发短促的纵向爆发。该预设不修改「雨境」9 的雨丝和玻璃水珠行为。

## 边界与约束

- 复用 Mineradio 当前的 Three.js `scene`、主 renderer、主循环和已有 `bass`、`mid`、`treble`、`beatPulse` 状态。
- 不创建第二个 Canvas，不创建第二个 `requestAnimationFrame`，不引入 Rainform 代码或资产。
...
模块维护一个固定对象池：18 个横向雨瀑列、每列 64 个点，共约 1152 个粒子。几何体包含 `aColumn`、`aSeed`、`aAlpha` 三个属性，顶点着色器根据列索引和时间计算列间起伏、拍点爆发、低频高度与高频点径；片元着色器使用竖向椭圆点形状和柔和边缘绘制透明雨线。
...
audio analyser -> bass/mid/treble/beatPulse
                  ...
                  smooth tracking
                         |
        shader uniforms + fixed rain columns
                         |
                   transparent scene
```
...
- 本轮不把 Rainform 的天气 API、数据结构或代码移入 Mineradio。
