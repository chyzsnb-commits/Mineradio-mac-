...
| 2026-09-07 | 追加到 PR #126：加入中英文界面切换与持久化语言偏好，覆盖主界面、播放器、队列、视觉控制台、登录和壁纸控制；保留歌曲/歌词等外部内容原文 | `node --check public/js/i18n.js`、`node --test scripts/test-i18n.js` 通过；完整安装包待重新冻结 Safe Storage 链后验证 |
...
- [x] PR #126 国际化候选包已完成整体构建与 preflight；Safe Storage 安装在 prepare 前失败后已恢复原 `/Applications/Mineradio.app`，未破坏登录态。
- [ ] 从最新 `4007111` 重新构建并在独立窗口验证安全迁移；本轮只保留代码推送，未替换安装版。
...
- [ ] 处理源码完整自动门禁已有的 4 个失败（284/288）：上游音频中断测试偶发失败、3 个 macOS 工作流断言；汽水策略与本次专项已通过，不为通过测试重新开启汽水。
