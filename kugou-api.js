...
const KUGOU_RANK_HOME = 'https://www.kugou.com/yy/rank/home/';
const KUGOU_MOBILE_API = 'http://mobilecdn.kugou.com/api/v3';
const KUGOU_HEADERS = {
...
const kugouToplistCache = createKugouTtlCache(16, 10 * 60 * 1000);
const kugouDetailCache = createKugouTtlCache(48, 10 * 60 * 1000);
...
function mapKugouSearchItem(item) {
  ...
  return {
    ...
    kugouRank: Number(item.rank || item.Rank || 0) || 0,
...
function parseKugouFileName(value) {
  const text = stripKugouHtml(value || '');
  const parts = text.split(/\s+-\s+/);
  return parts.length > 1
    ? { artist: parts.shift().trim(), name: parts.join(' - ').trim() }
    : { artist: '', name: text };
}

function mapKugouPublicTrack(item) {
  item = item || {};
  const parsed = parseKugouFileName(item.filename || item.FileName || '');
  const artist = stripKugouHtml(item.singername || item.author_name || item.SingerName || parsed.artist || '');
  const name = stripKugouHtml(item.songname || item.SongName || parsed.name || '');
  return mapKugouSearchItem({
    FileHash: item.hash || item.Hash || '',
    SongName: name,
    SingerName: artist,
    SingerId: item.singer_id != null ? [item.singer_id] : (item.SingerId || []),
    AlbumID: item.album_id || item.AlbumID || '',
    AlbumName: item.album_name || item.AlbumName || '',
    Image: item.cover || item.Image || (item.trans_param && item.trans_param.union_cover) || '',
    Duration: item.duration || item.timeLen || item.Duration || 0,
    Privilege: item.privilege != null ? item.privilege : item.Privilege,
    MixSongID: item.album_audio_id || item.MixSongID || '',
    Audioid: item.audio_id || item.Audioid || '',
    HQFileHash: item['320hash'] || item.HQFileHash || '',
    SQFileHash: item.sqhash || item.SQFileHash || '',
    ResFileHash: item.hash_high || item.ResFileHash || '',
    rank: item.rank || item.Rank || 0,
  });
}

function extractKugouPageGlobal(html, name) {
  const match = String(html || '').match(new RegExp('global\\.' + name + '\\s*=\\s*([\\s\\S]*?);(?:\\r?\\n|<\\/script>)'));
  if (!match) return null;
  return JSON.parse(match[1]);
}

async function fetchKugouRankPage(rankId) {
  const id = String(rankId || '8888').replace(/[^0-9]/g, '') || '8888';
  return kugouToplistCache.wrap('rank:' + id, 5 * 60 * 1000, async () => {
    const html = await requestText(KUGOU_RANK_HOME + '1-' + id + '.html?from=rank', { headers: KUGOU_HEADERS });
    return {
      id,
      features: extractKugouPageGlobal(html, 'features') || [],
      rankInfo: extractKugouPageGlobal(html, 'rankInfo') || {},
    };
  });
}
...
async function handleKugouRankList() {
  const definitions = [
    { id: '8888', name: '酷狗 TOP500', cover: 'https://imge.kugou.com/mcommon/400/20200909/20200909163138363620.jpg' },
    { id: '6666', name: '酷狗飙升榜', cover: 'https://imge.kugou.com/mcommon/400/20200909/20200909163323564315.jpg' },
  ];
  const settled = await Promise.allSettled(definitions.map(item => fetchKugouRankPage(item.id)));
  const toplists = definitions.map((item, index) => {
    const hit = settled[index].status === 'fulfilled' ? settled[index].value : null;
    return Object.assign({}, item, {
      provider: 'kugou',
      trackCount: hit && hit.features.length || 0,
      available: !!(hit && hit.features.length),
    });
  }).filter(item => item.available);
  return { provider: 'kugou', toplists };
}

async function handleKugouRankTracks(rankId, limit) {
  const page = await fetchKugouRankPage(rankId);
  const max = Math.max(1, Math.min(Number(limit) || 100, 500));
  const tracks = page.features.slice(0, max).map((item, index) => mapKugouPublicTrack(Object.assign({ rank: index + 1 }, item))).filter(song => song.name && song.id);
  const info = page.rankInfo || {};
  return {
    provider: 'kugou',
    playlist: {
      provider: 'kugou',
      id: page.id,
      name: stripKugouHtml(info.rankname || info.rank_name || '') || (page.id === '6666' ? '酷狗飙升榜' : '酷狗 TOP500'),
      cover: kugouCoverUrl(info.imgurl || info.img || '', 300),
...
async function handleKugouAlbumDetail(albumId, limit) {
  const id = String(albumId || '').trim();
  if (!id) return { provider: 'kugou', error: 'MISSING_ALBUM_ID', album: null, songs: [] };
  const max = Math.max(1, Math.min(Number(limit) || 120, 200));
  return kugouDetailCache.wrap('album:' + id + ':' + max, 10 * 60 * 1000, async () => {
    const json = await requestJson(KUGOU_MOBILE_API + '/album/song?albumid=' + encodeURIComponent(id) + '&page=1&pagesize=' + max, { headers: KUGOU_HEADERS });
    const data = json && json.data || {};
    const songs = (Array.isArray(data.info) ? data.info : []).map(mapKugouPublicTrack).filter(song => song.name && song.id);
    const first = songs[0] || {};
    return {
      provider: 'kugou',
      album: {
        provider: 'kugou', id, name: first.album || '', cover: first.cover || '',
        artist: first.artist || '', artistId: first.artistId || '', trackCount: Number(data.total || 0) || songs.length,
      },
      total: Number(data.total || 0) || songs.length,
      songs,
    };
  });
}

async function handleKugouArtistDetail(artistId, limit) {
  const id = String(artistId || '').trim();
  if (!id) return { provider: 'kugou', error: 'MISSING_ARTIST_ID', artist: null, songs: [] };
  const max = Math.max(1, Math.min(Number(limit) || 80, 120));
  return kugouDetailCache.wrap('artist:' + id + ':' + max, 10 * 60 * 1000, async () => {
    const [infoJson, songsJson] = await Promise.all([
      requestJson(KUGOU_MOBILE_API + '/singer/info?singerid=' + encodeURIComponent(id), { headers: KUGOU_HEADERS }),
      requestJson(KUGOU_MOBILE_API + '/singer/song?singerid=' + encodeURIComponent(id) + '&page=1&pagesize=' + max, { headers: KUGOU_HEADERS }),
    ]);
    const info = infoJson && infoJson.data || {};
    const data = songsJson && songsJson.data || {};
    const songs = (Array.isArray(data.info) ? data.info : []).map(mapKugouPublicTrack).filter(song => song.name && song.id);
    return {
      provider: 'kugou',
      artist: {
        provider: 'kugou', id, name: stripKugouHtml(info.singername || '') || (songs[0] && songs[0].artist) || '',
        avatar: kugouCoverUrl(info.imgurl || info.img || '', 300), profile: stripKugouHtml(info.profile || ''),
        musicSize: Number(info.songcount || data.total || 0) || songs.length,
      },
      total: Number(data.total || info.songcount || 0) || songs.length,
      songs,
    };
  });
}

async function kugouSearch(keywords, limit, cookie) {
  const auth = extractKugouAuth(cookie);
  const u = new URL(KUGOU_SEARCH_URL);
  u.searchParams.set('keyword', keywords);
  u.searchParams.set('page', '1');
  u.searchParams.set('pagesize', String(Math.max(1, Math.min(limit || 8, 20))));
...
async function handleKugouSearch(keywords, limit, cookie) {
  const kw = String(keywords || '').trim();
  const lim = Math.max(1, Math.min(Number(limit) || 10, 20));
  if (!kw) return [];
  const cacheKey = kw.toLowerCase() + ':' + lim;
  return kugouSearchCache.wrap(cacheKey, null, async () => {
    console.log('[KugouSearch]', kw, 'limit:', lim);
    return kugouSearch(kw, lim, cookie);
  });
}
...
async function handleKugouGuessLike(cookie, limit) {
  ...
  try {
    const songs = await kugouSearch('热门推荐', Math.min(limit, 10), cookie);
    if (songs.length) {
      return { provider: 'kugou', loggedIn: true, songs: songs.slice(0, limit), fallback: true, updatedAt: Date.now() };
    }
  } catch (e) {
    console.warn('[KugouGuessLike] search fallback:', e.message);
  }
