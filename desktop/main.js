...
} = require('./provider-load-error');
const {
  defaultFetchText,
  kugouProxySearch,
  kugouProxyPlaylistTracks,
  kugouProxyUserPlaylists,
} = require('./kugou-main-proxy');
...
let macWindowMaximized = false;
let macWindowRestoreBounds = null;
...
const APP_ICON = path.join(__dirname, '..', 'build', process.platform === 'darwin' ? 'icon.icns' : 'icon.ico');
const PROVIDER_LOGIN_CONFIG = {
  netease: {
  ...
  qq: {
    ...
    title: 'QQ 音乐登录',
    domains: ['qq.com', 'qqmusic.qq.com'],
    priority: ['uin', 'qqmusic_uin', 'wxuin', 'login_type', 'qm_keyst', 'qqmusic_key', 'music_key', 'p_skey', 'skey', 'psrf_qqopenid', 'psrf_qqunionid', 'psrf_qqaccess_token', 'psrf_qqrefresh_token', 'wxopenid', 'wxunionid', 'wxrefresh_token', 'wxskey', 'p_uin', 'ptcz', 'RK'],
  },
  kugou: {
    partition: 'persist:mineradio-kugou-login',
    url: 'https://www.kugou.com',
    title: '酷狗音乐登录',
    domains: ['kugou.com', 'kgimg.com', 'kugoucdn.com'],
    priority: ['KugooID', 'KuGoo', 'token', 'KugooToken', 'kg_mid', 'kg_dfid', 'kg_dfid_collect', 'kg_mid_temp', 'userid', 'dfid'],
  },
};
const NETEASE_LOGIN_PARTITION = PROVIDER_LOGIN_CONFIG.netease.partition;
const NETEASE_LOGIN_URL = PROVIDER_LOGIN_CONFIG.netease.url;
const QQ_LOGIN_PARTITION = PROVIDER_LOGIN_CONFIG.qq.partition;
const QQ_LOGIN_URL = PROVIDER_LOGIN_CONFIG.qq.url;
const TITLE_BAR_STYLE = process.platform === 'darwin' ? 'hiddenInset' : 'default';
...
async function ensureLocalServer() {
  ...
  process.env.MINERADIO_USER_DATA_DIR = userDataDir;
  process.env.COOKIE_FILE = path.join(userDataDir, '.cookie');
  process.env.QQ_COOKIE_FILE = path.join(userDataDir, '.qq-cookie');
...
function getWindowState(win) {
  ...
  return {
    isMaximized: process.platform === 'darwin' ? macWindowMaximized : win.isMaximized(),
...
function kugouCookieHasLogin(cookieText) {
  const obj = parseCookieHeader(cookieText);
  return !!(obj.KugooID && (obj.token || obj.KugooToken || obj.KuGoo));
}

function providerCookieHasLogin(provider, cookieText) {
  if (provider === 'netease') return neteaseCookieHasLogin(cookieText);
  if (provider === 'qq') return qqCookieHasLogin(cookieText);
  if (provider === 'kugou') return kugouCookieHasLogin(cookieText);
  return false;
}

function providerCookieDomainAllowed(provider, domain) {
  const config = PROVIDER_LOGIN_CONFIG[provider];
  const normalized = String(domain || '').replace(/^\./, '').toLowerCase();
  return !!(config && config.domains.some((allowed) => normalized === allowed || normalized.endsWith(`.${allowed}`)));
}
...
async function readProviderLoginCookieHeader(provider, cookieSession) {
  const config = PROVIDER_LOGIN_CONFIG[provider];
  if (!config) throw new Error(`Unsupported login provider: ${provider}`);
  const cookies = await cookieSession.cookies.get({});
  return buildCookieHeaderFor(
    cookies,
    (domain) => providerCookieDomainAllowed(provider, domain),
    config.priority
  );
}

async function readKugouLoginCookieHeader() {
  return readProviderLoginCookieHeader('kugou', session.fromPartition(PROVIDER_LOGIN_CONFIG.kugou.partition));
}

async function fetchKugouProxyText(url, options = {}) {
  const cookie = options.cookie != null ? String(options.cookie || '') : await readKugouLoginCookieHeader();
...
async function scrapeKugouPageHtml(url) {
  return new Promise((resolve, reject) => {
    const scraper = new BrowserWindow({
      ...
      height: 900,
      webPreferences: {
        partition: PROVIDER_LOGIN_CONFIG.kugou.partition,
    ...
    const finish = (error, html) => {
      if (settled) return;
      settled = true;
      try { if (!scraper.isDestroyed()) scraper.destroy(); } catch (_) {}
      if (error) reject(error);
      else resolve(html || '');
    };
    const timer = setTimeout(() => finish(new Error('KUGOU_SCRAPE_TIMEOUT')), 12000);
    scraper.webContents.once('did-finish-load', async () => {
      try {
        const html = await scraper.webContents.executeJavaScript('document.documentElement ? document.documentElement.outerHTML : ""', true);
        clearTimeout(timer);
        finish(null, html);
      } catch (error) {
        clearTimeout(timer);
    ...
    scraper.webContents.once('did-fail-load', (_event, code, description, validatedURL) => {
      clearTimeout(timer);
      const error = new Error(description || 'KUGOU_SCRAPE_FAILED');
      error.code = code;
      error.validatedURL = validatedURL;
      finish(error);
    });
    scraper.loadURL(url).catch((error) => {
      clearTimeout(timer);
      finish(error);
    });
  });
...
async function openProviderLoginWindow(provider, owner) {
  const config = PROVIDER_LOGIN_CONFIG[provider];
  if (!config) return { ok: false, error: 'UNSUPPORTED_LOGIN_PROVIDER' };
  const cookieSession = session.fromPartition(config.partition);
  const initialCookie = await readProviderLoginCookieHeader(provider, cookieSession);
  if (providerCookieHasLogin(provider, initialCookie)) return { ok: true, cookie: initialCookie, reused: true };
  ...
  return new Promise((resolve) => {
    ...
    const loginWindow = new BrowserWindow({
      width: provider === 'netease' ? 940 : 900,
      height: provider === 'netease' ? 760 : 720,
      minWidth: 760,
      minHeight: 560,
    ...
    const finish = (result) => {
      ...
      cookieSession.cookies.removeListener('changed', onCookieChanged);
      if (!loginWindow.isDestroyed()) loginWindow.close();
      resolve(result);
    };
    const checkCookies = async () => {
      try {
        const cookie = await readProviderLoginCookieHeader(provider, cookieSession);
        const ready = provider === 'qq' ? qqCookieHasPlaybackLogin(cookie) : providerCookieHasLogin(provider, cookie);
        if (ready) return finish({ ok: true, cookie });
        if (provider === 'qq' && qqCookieHasLogin(cookie) && !warmupStarted) {
          warmupStarted = true;
          setTimeout(() => {
            if (!settled && !loginWindow.isDestroyed()) {
              loginWindow.loadURL('https://y.qq.com/n/ryqq/player').catch(() => {});
            }
          }, 900);
        }
      } catch (e) {
        console.warn(`${config.title} cookie check failed:`, e.message);
      }
    };
    const onCookieChanged = () => {
      checkCookies();
    };
    cookieSession.cookies.on('changed', onCookieChanged);

    loginWindow.webContents.setWindowOpenHandler(({ url }) => {
      if (isKugouPrivateProtocolUrl(url)) {
        checkCookies();
        return { action: 'deny' };
      }
    ...
    loginWindow.webContents.on('will-navigate', (event, url) => {
      if (!isKugouPrivateProtocolUrl(url)) return;
    ...
    loginWindow.webContents.on('did-fail-load', (
      ...
      if (!isMainFrame || !isIgnorableProviderLoadError(provider, {
        errorCode,
      ...
      loginWindow.webContents.executeJavaScript(`
        setTimeout(() => {
          ...
          const node = nodes.find((item) => {
            ...
            return /登录|登陆|扫码登录/.test(text) && rect.width > 0 && rect.height > 0;
          });
          if (node) node.click();
        }, 800);
      `, true).catch(() => {});
    ...
    loginWindow.on('closed', async () => {
      ...
      try {
        const cookie = await readProviderLoginCookieHeader(provider, cookieSession);
        resolve(providerCookieHasLogin(provider, cookie)
          ? { ok: true, cookie, partial: provider === 'qq' && !qqCookieHasPlaybackLogin(cookie) }
          : { ok: false, cancelled: true, message: `${config.title}窗口已关闭` });
      } catch (e) {
        resolve({ ok: false, error: e.message || `${config.title}窗口已关闭` });
      }
    });
    pollTimer = setInterval(checkCookies, 1200);
    loginWindow.loadURL(config.url).catch((error) => {
      if (isIgnorableProviderLoadError(provider, error)) {
        checkCookies();
        if (!loginWindow.isDestroyed() && !loginWindow.isVisible()) loginWindow.show();
        return;
      }
...
async function clearProviderLoginSession(provider) {
  const config = PROVIDER_LOGIN_CONFIG[provider];
  if (!config) return { ok: false, error: 'UNSUPPORTED_LOGIN_PROVIDER' };
  const cookieSession = session.fromPartition(config.partition);
...
function applyWindowedBounds(win) {
  if (!win || win.isDestroyed()) return;
  macWindowMaximized = false;
  macWindowRestoreBounds = null;
...
function toggleMacWindowMaximize(win) {
  if (!win || win.isDestroyed()) return;
  if (macWindowMaximized && macWindowRestoreBounds) {
    const restoreBounds = macWindowRestoreBounds;
    macWindowMaximized = false;
    macWindowRestoreBounds = null;
    win.setBounds(restoreBounds, true);
  } else {
    const display = screen.getDisplayMatching(win.getBounds());
    macWindowRestoreBounds = win.getBounds();
    macWindowMaximized = true;
    win.setBounds(display.workArea, true);
  }
  sendWindowState(win);
}

function scaleWindowBounds(win, scale) {
  if (!win || win.isDestroyed()) return;
  if (win.isFullScreen() || win.isMaximized() || macWindowMaximized) return;
  const bounds = win.getBounds();
  const display = screen.getDisplayMatching(bounds);
  const area = display.workArea || display.bounds;
  const width = Math.min(area.width, Math.max(MIN_WINDOWED_WIDTH, Math.round(bounds.width * scale)));
  const height = Math.min(area.height, Math.max(MIN_WINDOWED_HEIGHT, Math.round(bounds.height * scale)));
  const x = Math.round(Math.max(area.x, Math.min(
    bounds.x - (width - bounds.width) / 2,
    area.x + area.width - width,
  )));
  const y = Math.round(Math.max(area.y, Math.min(
...
// 窗口缩放功能
ipcMain.handle('desktop-window-zoom-in', (event) => {
  scaleWindowBounds(getSenderWindow(event), 1.1);
});

ipcMain.handle('desktop-window-zoom-out', (event) => {
  scaleWindowBounds(getSenderWindow(event), 0.9);
});

ipcMain.handle('desktop-window-zoom-reset', (event) => {
...
ipcMain.handle('netease-music-open-login', async (event) => {
  return openProviderLoginWindow('netease', getSenderWindow(event));
});

ipcMain.handle('netease-music-clear-login', async () => {
  return clearProviderLoginSession('netease');
});

ipcMain.handle('qq-music-open-login', async (event) => {
  return openProviderLoginWindow('qq', getSenderWindow(event));
});

ipcMain.handle('qq-music-clear-login', async () => {
  return clearProviderLoginSession('qq');
});

ipcMain.handle('kugou-music-open-login', async (event) => {
  return openProviderLoginWindow('kugou', getSenderWindow(event));
});

ipcMain.handle('kugou-music-clear-login', async () => {
  return clearProviderLoginSession('kugou');
});

ipcMain.handle('kugou-proxy-search', async (_event, keywords, limit) => {
  try {
    const cookie = await readKugouLoginCookieHeader();
    const songs = await kugouProxySearch(fetchKugouProxyText, cookie, keywords, limit);
    return { ok: true, provider: 'kugou', songs };
  } catch (error) {
    console.error('[Kugou:proxy-search]', {
      message: error && error.message || 'Unknown error',
      status: error && error.statusCode || 0,
      body: String(error && error.body || '').slice(0, 2000),
    });
    return { ok: false, provider: 'kugou', error: error.message || 'KUGOU_PROXY_SEARCH_FAILED', songs: [] };
  }
});

ipcMain.handle('kugou-proxy-user-playlists', async () => {
  try {
    const cookie = await readKugouLoginCookieHeader();
    const data = await kugouProxyUserPlaylists(fetchKugouProxyText, cookie, scrapeKugouPageHtml);
    return Object.assign({ ok: true }, data);
  } catch (error) {
    console.error('[Kugou:proxy-user-playlists]', {
      message: error && error.message || 'Unknown error',
      status: error && error.statusCode || 0,
      body: String(error && error.body || '').slice(0, 2000),
    });
    return { ok: false, provider: 'kugou', error: error.message || 'KUGOU_PROXY_USER_PLAYLISTS_FAILED', playlists: [] };
  }
});

ipcMain.handle('kugou-proxy-playlist-tracks', async (_event, playlistId) => {
  try {
    const cookie = await readKugouLoginCookieHeader();
    const data = await kugouProxyPlaylistTracks(fetchKugouProxyText, cookie, playlistId);
    return Object.assign({ ok: true }, data);
  } catch (error) {
    console.error('[Kugou:proxy-playlist-tracks]', {
      message: error && error.message || 'Unknown error',
      status: error && error.statusCode || 0,
      body: String(error && error.body || '').slice(0, 2000),
    });
    return { ok: false, provider: 'kugou', error: error.message || 'KUGOU_PROXY_PLAYLIST_TRACKS_FAILED', tracks: [], playlist: { id: String(playlistId || '') } };
  }
});
...
async function createWindow() {
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    if (input.type === 'keyDown' && (input.key === 'Escape' || input.code === 'Escape') && (mainWindow.isFullScreen() || windowFullscreenActive || macWindowMaximized)) {
      event.preventDefault();
  ...
  const template = [
    {
      label: app.name,
      submenu: [
        { role: 'about', label: '关于 Mineradio' },
        { type: 'separator' },
        { role: 'services', label: '服务' },
        { type: 'separator' },
        { role: 'hide', label: '隐藏 Mineradio' },
        { role: 'hideOthers', label: '隐藏其他' },
        { role: 'unhide', label: '显示全部' },
        { type: 'separator' },
        { role: 'quit', label: '退出 Mineradio' }
      ]
    },
    {
      label: '编辑',
