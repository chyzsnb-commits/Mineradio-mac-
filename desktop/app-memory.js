...
function trimAppWorkingSets(pids) {
  if (!isWin) {
    // macOS 等平台无 EmptyWorkingSet；主进程能做的软压缩是触发 V8 GC(需 --expose-gc)。
    // renderer GC + 缓存清理由 main.js 按平台分派完成，这里不再对 mac 早退。
    let gc = false;
    try { if (typeof global.gc === 'function') { global.gc(); gc = true; } } catch (e) {}
    return Promise.resolve({ ok: true, soft: true, scope: 'app', platform: process.platform, gc, trimmed: 0 });
  }
