...
function makeTempPath(label, ext) {
  return path.join(os.tmpdir(), 'mineradio-' + label + '-' + process.pid + '-' + Date.now() + '.' + ext);
}
...
function isProcessElevated() {
  ...
  return runPowerShellFile(scriptPath, 10000).then((value) => !!value).catch(() => false).finally(() => safeUnlink(scriptPath));
}
